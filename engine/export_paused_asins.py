...
PRODUCT_CSV = export_reader.newest_catalog_file(POD)
if not PRODUCT_CSV:
    raise SystemExit(f"No product export found in {POD}")
...
# Read through export_reader: MerchFlow column names find nothing on a Snap
# chunk, and the catalog is a merge of several files now.
for prod in export_reader.catalog_rows(POD):
    asin = prod.get("asin")
