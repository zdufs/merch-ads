...
OUTDIR = os.path.join(paths.REPO_ROOT, "outputs"); os.makedirs(OUTDIR, exist_ok=True)
...
_st_gate = db.snapshot_gate(conn, "search_term_perf")
end = _st_gate["date"]
...
def _promotion_target(agid):
    ...
    if markets.is_kdp():
        ...
        return 0.0
    if d and d.get("asin"):
        e = products.get_design_econ(d.get("product_type") or pm.get(str(agid)),
                                     price=d.get("list_price"))
        # A design we could not price has no target ACOS. get_design_econ answers
        # with a display fallback, and promotion is a LIVE write that creates
        # campaigns and keywords — so a term was being called profitable against
        # a threshold belonging to no real product. None means "not eligible",
        # which is what an unpriceable design should be.
        if not e.get("known_price", True):
            return None
        return e["target_acos"]
    e = products.get_econ(pm.get(str(agid)))
    if not e.get("known"):
        return None
    return e["target_acos"]


# Fail closed on stale evidence. This read anchored to the table's own newest
# snapshot, which was the right table but no freshness check at all: after four
# nights of a failing search-term report it went on nominating winners from the
# old cumulative snapshot, and the promotion step downstream is a LIVE write. An
# empty table was worse still, printing "candidates this run: 0" — which reads as
# "nothing qualified" rather than "there is nothing to read".
rows = cur.execute(
    ...
    (end, MIN_ORDERS)).fetchall() if _st_gate["ok"] else []
...
for st, cid, agid, clk, orders, cost, sales, acos in rows:
    ...
    if tgt is None:
        skipped_unpriced += 1
        continue          # economics unavailable: fail closed, never promote
    if acos <= tgt:
...
if not _st_gate["ok"]:
    print(f"  NO CANDIDATES — search-term evidence is not usable: {_st_gate['reason']}")
print(f"  candidates this run: {len(cand)}  | distinct terms not yet promoted: {len(ranked)}")
if skipped_unpriced:
    print(f"  {skipped_unpriced} converting term(s) skipped: their design's "
          f"economics could not be resolved, so no target ACOS applies.")
