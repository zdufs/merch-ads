...
per-type ACOS rules. Scavenger campaigns scoop up CHEAP CLICKS:
...
# NOTE: hardgoods (hats, mugs, water bottles, tumblers) ARE advertisable, but
...
# Listings of these types with no ad-eligible ASIN can't be advertised at all, so
# `load_build_specs` skips them and COUNTS the skip into the coverage report.
#
# That last sentence described the intent from the start and the code did the
# opposite: it fell back to the retail ASIN and submitted it. Measured on the
# 2026-08-25 run log, which is the first that recorded per-batch refusal reasons:
# SCAVENGER - Hats 1 submitted 194 ASINs and Amazon refused 194, and the Hats
# shard holds exactly 194 listings with no ad-safe ASIN. Every drinkware shard
# matched the same way (10 of 10, 1 of 1, and a control shard of 1,000 ad-safe
# ASINs that refused nothing). Every reason read `adEligibilityError
# AD_INELIGIBLE`. A refused ad never joins Amazon's live product-ad list, so
# `new_asins` proposed the identical set the next night, for sixty nights.
AD_SAFE_REQUIRED_TYPES = frozenset({
    "tumbler", "mug", "water_bottle",
    "printed_trucker_hat", "printed_baseball_hat", "sport_sun_visor",
})


def needs_ad_safe_asin(product_type):
    """True when this type can ONLY be advertised through the export's adAsins.

    Deliberately a type question, not a series one. A cohort that grows a new
    hardgood type has to declare it here, where the reason is written down,
    rather than inheriting the answer from whichever series it was filed under.
    """
    return (product_type or "") in AD_SAFE_REQUIRED_TYPES
...
def series_name(series=TEES_SERIES):
    """The series label as it appears in THIS market's campaign names.

    US keeps the legacy "US Tees" name (existing campaigns); other markets get a
    clean "Tees" — they live in their own market account, so there is no need for
    a country tag.

    This was inline in camp_name(), so anything else that needed to recognise a
    campaign it had built compared against the raw COHORTS label and missed.
    scav_cleanup did exactly that: outside US it read every legitimate
    'SCAVENGER - Tees N' as an orphan and would have PAUSED it.
    """
    import markets
...
def camp_name(n=1, series=TEES_SERIES):
    return f"{PREFIX}{series_name(series)} {n}"
