...
def apply_plan(plan, auto=False):
    ...
    conn = None
    # Three faults sat in six lines here. The connection was passed as the
    # positional `market` argument, so the gate judged a market named after a
    # sqlite3.Connection. Any exception became {"ok": True}, which is a gate that
    # fails OPEN — the opposite of every other gate in this engine. And the
    # per-book economics the plan already carries were never looked at, so a book
    # whose preview says its royalty is unknown was still given a live campaign
    # with a daily budget.
    import db
    try:
        conn = db.connect(ro=True)
    except Exception as e:
        return {"applied": False, "blocked": "econ_gate",
                "reasons": [f"could not open the market database: {e}"]}
    try:
        gate = products.econ_gate(conn=conn)
    except Exception as e:
        return {"applied": False, "blocked": "econ_gate",
                "reasons": [f"the economics gate could not be read: {e}"]}
    if not gate.get("ok", True):
        return {"applied": False, "blocked": "econ_gate", "reasons": gate.get("reasons")}
    # A book with no entry in kdp_books.json has no break-even, so nothing can
    # say whether a bid on it is profitable. kdp_econ fails closed on purpose;
    # creating its campaign anyway threw that away.
    unpriced = [c.get("name") or "?" for c in plan.get("campaigns", [])
                if not (c.get("econ") or {}).get("available")]
    if unpriced:
        return {"applied": False, "blocked": "econ_unavailable",
                "reasons": [f"no royalty entered for: {', '.join(sorted(set(unpriced))[:8])}"
                            + (" …" if len(set(unpriced)) > 8 else "")
                            + ". Add it in the KDP Books tab, or with"
                              " `appctl kdp-book --asin X --list-price P --royalty R`."]}
    if not auto:
    ...
    for c in plan["campaigns"]:
        ...
        if 0 not in ids:
            # A campaign Amazon refused. `partial` is what the reply's
            # `partials` list is built from, so without it this failure was
            # invisible above and the whole run still reported applied: true.
        ...
        if ag is None:
            # The worst outcome this builder can produce: a LIVE campaign with
            # a budget and no ad group under it. It can never serve, and the
            # next run finds the name in `existing` and skips the whole
            # downstream build — so it stays empty for good while every run
            # reports "already exists".
            made.append({"name": c["name"], "campaign_id": cid,
                         "created": "campaign_only", "partial": ["ad_group"],
                         "note": "campaign is LIVE with a budget and NO ad group — "
                                 "it cannot serve, and the next run will skip it "
                                 "by name. Delete or repair it by hand."})
            continue
        ...
        # The product-target negatives' response was thrown away entirely, so
        # they were neither counted nor flagged. Three states, not two: this
        # endpoint's success block is not one the engine parses anywhere else,
        # so a 2xx whose body cannot be read is UNCONFIRMED rather than failed.
        # An alarm that fires on every build gets muted.
        nt_ok, nt_unconfirmed = 0, False
        want_nt = len(c["negatives"]["product_targets"])
        if want_nt:
            nt_st, nt_js = client.create_negative_product_targets(
                [{"campaignId": cid, "adGroupId": ag, "asin": n["asin"]}
                 for n in c["negatives"]["product_targets"]])
            block = (nt_js or {}).get("negativeTargetingClauses") or {}
            ok_idx = {i["index"] for i in (block.get("success") or []) if "index" in i}
            err_idx = {i["index"] for i in (block.get("error") or []) if "index" in i}
            if nt_st in (200, 207) and (ok_idx or err_idx):
                nt_ok = len(ok_idx)
            elif nt_st in (200, 207):
                nt_unconfirmed = True
                nt_ok = want_nt        # not counted as a shortfall, but said below
        entry = {"name": c["name"], "campaign_id": cid, "ad_group_id": ag, "created": True,
                 ...
                 "negative_targets": f"{nt_ok}/{want_nt}"}
        if nt_unconfirmed:
            entry["negative_targets_unconfirmed"] = True
        partial = [name for name, ok, want in
                   (("product_ads", pa_ok, len(c["product_ads"])),
                    ("keywords", kw_ok, len(c["keywords"])),
                    ("negatives", neg_ok, len(c["negatives"]["keywords"])),
                    ("negative_targets", nt_ok, want_nt)) if ok < want]
        if partial:
            entry["partial"] = partial
        made.append(entry)
    partials = [m["name"] for m in made if m.get("partial")]
    # `applied` used to be a bare True whatever happened underneath, so a run
    # where every campaign came back campaign_only still read as a clean
    # success. It answers the question it is asked: did everything land.
    return {"applied": not partials, "market": MARKET, "campaigns": made,
            "partials": partials,
            "skipped_existing": [m["name"] for m in made if m.get("skipped")]}
