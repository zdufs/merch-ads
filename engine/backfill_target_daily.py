    ...
    while pending and waited <= MAX_WAIT:
        for span in list(pending):
            ...
            if status == "COMPLETED":
                ...
                # This span was dropped from `pending` without ever joining
                # `failed`, so a run where Amazon refused EVERY report still
                # reached `return 0`. The Monday nightly then recorded the
                # backfill as passed while target_daily was not refreshed.
