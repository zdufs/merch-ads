...
Read-only vs Amazon (it's a report). Writes TWO local tables: daily_totals and,
from the same rows, campaign_daily via db.store_campaign_daily. This line used
to say "only the local daily_totals table", which is how it reads at a glance
and is not what it does — and on 2026-08-24 that sentence sent an investigation
down the wrong path while working out why four EU markets disagreed with their
own trailing-30 snapshot. The campaign half is the reason a market that misses
this step keeps last week's attribution on every per-campaign screen.
...
def main():
    ...
    waited = 0
    banked = 0
    failed = []          # spans Amazon refused outright
    pending = dict(active)
    while pending and waited <= MAX_WAIT:
        for span in list(pending):
            ...
            if status == "COMPLETED":
                ...
                # A terminal refusal from Amazon. This span's days were NOT
                # refreshed, so it has to reach the exit code — the Monday
                # nightly records this step as passed otherwise, and health
                # goes on showing stale history with nothing in
                # last_run_status.json to say why.
    ...
    if failed:
        print(f"  {len(failed)} report(s) FAILED or were CANCELLED by Amazon — "
              f"those days were not refreshed.")
    if pending:
        print(f"  {len(pending)} report(s) still generating — saved; re-run to resume.")
    print(f"Done: {banked} days banked.")
    # Same contract as backfill_target_daily: anything not banked is a non-zero
    # exit, so the nightly's step tracker records it.
