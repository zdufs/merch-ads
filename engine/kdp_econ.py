...
# Amazon POLICY, and policy moves. Confirm these against the KDP help pages or
# your own dashboard before trusting a computed ebook royalty; the per-book
# `royalty` entered from the dashboard bypasses this whole path and is what the
# operator should use.
EBOOK_70_MIN = 2.99
EBOOK_70_MAX = 9.99
EBOOK_DELIVERY_PER_MB = 0.15
_EBOOK_DELIVERY_PER_MB = EBOOK_DELIVERY_PER_MB   # old name, kept for callers
...
def ebook_royalty(list_price, file_size_mb=1.0):
    """US Kindle royalty for one sale.

    Amazon's published shape is 70% OF (list price MINUS the delivery cost) —
    not 70% of the price with delivery taken off afterwards. This computed
    70% * price - delivery, which is a different number and always the larger
    one, because it never discounts the delivery by the 30% Amazon keeps.

    The 35% band pays no delivery cost at all, which the old code got right by
    accident: it simply never reached that line with a delivery term.

    The two constants below are Amazon POLICY and policy moves. They are named
    so there is one place to correct them, and `kdp-book --royalty` remains the
    accurate path: a figure read off the KDP dashboard beats any formula here
    and is what book_econ prefers.
    """
    if EBOOK_70_MIN <= list_price <= EBOOK_70_MAX:
        delivery = EBOOK_DELIVERY_PER_MB * (file_size_mb or 0)
        return round(0.70 * max(list_price - delivery, 0.0), 2)
