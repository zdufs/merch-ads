...
def build(conn):
    # Only bids that actually LANDED describe the live bid. A rejected bid_change
    # row used to count as the newest move, so the "current" bid was a number
    # Amazon had refused: the reset then lowered a bid from a value that never
    # existed, and it read as a correct reset in the audit trail.
    rows = conn.execute(
        "SELECT entity_id, prev_state, detail, rowid FROM writes_log "
        "WHERE action='bid_change' AND NOT " + db.FAILED_RESULT_SQL +
        " ORDER BY entity_id, rowid").fetchall()
    hist = {}
...
def apply(client, conn, plan):
    """Apply reset bids and mirror only confirmed, clamped values."""
    res = client.update_target_bids(
        [{"targetId": p["targetId"], "bid": p["new"]} for p in plan])
    accepted = ads_client.certain_ids(res, [p["targetId"] for p in plan])
    caps = {str(c["id"]): c["cap"]
            for c in (getattr(client, "last_clamps", None) or [])}
    written = {str(p["targetId"]): caps.get(str(p["targetId"]), p["new"])
               for p in plan}
    for p in plan:
        tid = str(p["targetId"])
        db.log_write(conn, "bid_change", "target", p["targetId"],
                     f"snap=reset {p['current']}->{written[tid]} (reset to original-10%)"
                     + (" [adjusted]" if tid in caps else ""),
                     str(p["current"]), "submitted" if tid in accepted else "failed")
    db.set_local_target_bids(
        conn, [(p["targetId"], written[str(p["targetId"])]) for p in plan
               if str(p["targetId"]) in accepted])
    return res, accepted, written
...
def main():
    ...
    res, accepted, _written = apply(client, conn, plan)
    ok = sum(1 for b in res if b["http"] in (200, 207))
    print(f"  reset {len(accepted)}/{len(plan)} bids "
          f"({ok}/{len(res)} batches accepted). See writes_log.")
