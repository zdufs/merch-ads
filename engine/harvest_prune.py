    ...
    for action, eid in conn.execute(
        """SELECT action, entity_id FROM writes_log
           WHERE action IN ('pause_keyword','undo_pause_keyword',
                            'pause_target','undo_pause_target') AND NOT """
        + db.FAILED_RESULT_SQL + " ORDER BY rowid"):
...
def build_plan(conn):
    ...
    cur = conn.cursor()
    # Fail closed on stale evidence, like phase2 and phase3. This read used to be
    # a bare MAX(date): a targeting report that had been failing for weeks still
    # produced pauses, because a CUMULATIVE snapshot keeps reading as a full
    # trailing-30 window however old it is. An empty table was worse — it read as
    # "every keyword is pulling its weight" rather than "there is no evidence".
    tg_gate = db.snapshot_gate(conn, "targeting_perf")
    end = tg_gate["date"]
    if not tg_gate["ok"]:
        print("TARGETING DATA STALE — no harvest-prune plan this run: "
              + tg_gate["reason"], file=sys.stderr)
        return end, []
    gate = products.econ_gate(conn=conn)
    if not gate["ok"]:
        # STDERR, never stdout. build_plan() is called IN-PROCESS by appctl's
        # harvest-prune, which promises exactly one JSON object on stdout. A
        # stray line here lands in the pipe ahead of the envelope, and the app
        # only survives it because its decoder rescans lines. Anything else
        # reading the contract — jq, a script, a future caller — gets garbage.
        # phase2_apply learned this the hard way; the same rule applies here.
...
def _pause_batch(client, conn, rows, api, action, etype):
    return pause_outcome(client, conn, rows, api, action, etype)["paused"]


def pause_outcome(client, conn, rows, api, action, etype):
    """Pause one kind of entity and REPORT what Amazon confirmed.

    Returns `{"requested", "paused", "failed", "unconfirmed"}`. The count alone
    cannot be read: a batch Amazon refused entirely returns paused 0, which is
    the same number a run with nothing to do returns, and the app printed that
    as a green "Paused 0 keywords." (found by review, 2026-08-24).

    `failed` is only what Amazon NAMED as rejected. Everything else it did not
    confirm — a transport failure, or a 207 with more errors than it could
    index — is `unconfirmed`, because "Amazon refused these" is a claim about
    Amazon and a dropped connection supports no such claim. Same split as
    appctl._applied_outcome, for the same reason.
    """
    if not rows:
        return {"requested": 0, "paused": 0, "failed": 0, "unconfirmed": 0}
    ids = [p["entity_id"] for p in rows]
    try:
        res = api(ids, "PAUSED")
    except Exception as e:
        # STDERR: _pause_batch is called IN-PROCESS by appctl's
        # harvest-prune-apply, which owes the caller one JSON object on stdout.
        print(f"  {etype}s: pause call raised {e!r}", file=sys.stderr)
        res = []
    accepted = ads_client.certain_ids(res, ids)
    for p in rows:
        landed = str(p["entity_id"]) in accepted
        db.log_write(conn, action, etype, p["entity_id"],
                     f"{p['label']} ({p['reason']})" + (p.get("econ_sfx") or ""),
                     "ENABLED", "submitted" if landed else "failed")
    reachable = bool(res) and all(b.get("http") in (200, 207) for b in res)
    outcomes = ads_client.item_outcomes(res, ids)
    paused = sum(1 for o in outcomes if o == "accepted")
    failed = sum(1 for o in outcomes if o == "failed") if reachable else 0
    print(f"  {etype}s: paused {paused}/{len(rows)}", file=sys.stderr)
    return {"requested": len(rows), "paused": paused, "failed": failed,
            "unconfirmed": max(0, len(rows) - paused - failed)}
...
def apply(client, conn, plan):
    ...
    print(f"  paused {n} total. See writes_log.", file=sys.stderr)
