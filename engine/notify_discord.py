...
def load_env(path=os.path.join(HERE, ".env")):
    env = {}
    # Discord is optional and the digests ship OFF, so a data folder with no
    # .env is an ordinary state here, not a fault. Report it and stop.
    if not os.path.exists(path):
        sys.exit(f"No .env found at {path} — nothing to read the Discord "
                 "webhook from. Discord notifications are optional; see "
                 "docs/SETUP.md if you want them.")
...
# Only writes that LANDED, and only if they are from today. This counted every
# logged row, so a night when Amazon rejected three bid changes announced three
# bids applied — and on a night that wrote nothing at all it replayed the last
# day that did, under the heading "auto-applied this run".
applied = {}
last_day = cur.execute(
    "SELECT substr(MAX(applied_at),1,10) FROM writes_log WHERE NOT "
    + db.FAILED_RESULT_SQL).fetchone()[0]
today = _dt.date.today().isoformat()
if last_day == today:
    for a, n in cur.execute(
        "SELECT action,COUNT(*) FROM writes_log WHERE substr(applied_at,1,10)=? "
        "AND NOT " + db.FAILED_RESULT_SQL + " GROUP BY action", (last_day,)):
        applied[a] = n
...
top = movers[:3]
# SYM, not a hardcoded dollar. Every other money line in this digest already
# used the market's symbol; the movers line reported a UK campaign's swing as
# dollars — the right number wearing the wrong currency, which is worse than an
# obviously missing one.
mv_txt = "\n".join(f"• {n[:34]}  {'+' if d>=0 else ''}{SYM}{d:,.0f}" for d, n in top) or "—"
...
try:
    ...
    # Exit non-zero. This swallowed every webhook failure and exited 0, so a
    # rotated or revoked webhook meant no digest reached anyone for any market
    # while the nightly's step tracker recorded every notify step as a success —
    # silence that looks exactly like a quiet night.
