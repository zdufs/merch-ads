...
def owned_cross_sell_royalty(conn, market=None, econ_conn=None):
    ...
    def per_unit_royalty(asin):
        ...
        if key not in roy_cache:
            ...
            # get_design_econ answers with a DISPLAY fallback when it could not
            # price the design — an unsupported price, an unmapped type. That
            # fallback is a positive number, so it passed the test below and was
            # spent as if it were real. This map SPARES ad groups from being
            # paused, so an invented royalty keeps a loss-making design running.
            known = econ.get("known_price", econ.get("known", True))
            roy_cache[key] = roy if (known and roy and roy > 0) else None
