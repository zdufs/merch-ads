...
def parse_args(argv=None):
    ap = argparse.ArgumentParser(description="Bank this market's daily and "
                                             "month-to-date totals.")
    # Same reason as phase0_pull: a catch-up asks for every market first, then
    # collects. The report ids are banked, so nothing is wasted either way.
...
def main(argv=None):
    ...
    while pending and waited <= args.max_wait:
        for period in list(pending):
            ...
            if status == "COMPLETED":
                ...
                # A day Amazon refused to build is a HOLE, and a hole in
                # daily_totals / campaign_daily silences every rolling-window
                # rule that spans it. This used to be dropped from `pending` and
                # counted nowhere, so the process exited zero and the nightly
                # recorded a healthy step over a missing day.
                print(f"  {period}: {status} — NOT banked")
    ...
    # A report still GENERATING is normal — the next run resumes it. A report
    # Amazon marked FAILED is not, and the nightly's step tracker only ever
    # hears about it through the exit code.
    if failed:
        print(f"\n⚠️ NOT BANKED: {', '.join(failed)} — Amazon marked those reports"
              f" failed, so those days are missing. Re-run to retry.")
        sys.exit(1)
