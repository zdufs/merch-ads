...
payload = dict(snaps=snaps, latest=latest, st_latest=st_latest,
               types=sorted({b["type"] for b in byType}),
...
function renderTerms(){
 ...
 // D.latest is campaign_perf's date and these rows come from search_term_perf.
 // Two report jobs, two dates: the QUERY was fixed to read each table's own
 // newest snapshot and this label was not, so on any day the search-term job
 // had failed the panel showed older rows under today's date.
 $('stCount').textContent=rows.length+' terms (top 2000 by spend, '+(D.st_latest||'no data')+')';
