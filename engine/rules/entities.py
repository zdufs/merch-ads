...
# A ratio whose denominator is zero is UNDEFINED, and in this DSL an undefined
# value is NONE — it matches no numeric comparison, so a rule skips the row
# rather than acting on a number nobody measured. `_acos`, `_cpc` and `_roas`
# already worked that way. These two answered 0.0, and 0.0 is not "unknown", it
# is the worst possible score.
#
# That mattered because of WHICH WAY these two get compared. `cvr` and `ctr` are
# the metrics you write with `<`, because low is bad — so `IF adGroup.cvr < 8%`
# was TRUE for every ad group that had never been clicked. Measured on the live
# US account: 37,330 of them, against a rule whose action is `pause()`. The
# engine fails closed on unknown data everywhere else — economics, snapshots,
# rolling windows, batch writes — and failed open here, in the direction of
# switching ads off.
#
# `docs/rules-dsl.md` said all of these answered NONE, which is what the author
# of the guide believed and what every rule reads as though it does.
#
# Changing it moves nothing today, and that was checked rather than assumed:
# every rule in every market was previewed before and after, and all 53 counts
# are identical. Only `pause-kill-candidates` reads `cvr` at all, and it demands
# `clicks >= 15` first, so it never met the zero-click case.
#
# Note the case that does NOT change: clicks > 0 with no orders is a real,
# measured 0% conversion, and stays 0.0.
...
# Which snapshot table each entity kind is evaluated against.
#
# A bare MAX(date) over an EMPTY table returns None, every `date = ?` query then
# matches zero rows, and the rule reports `evaluated: 0, matched: 0` with
# `ok: True`. That is "no data" wearing the exact shape of "nothing qualified",
# and for a REVIEW rule it clears the pending queue as though the engine had
# looked and found nothing worth proposing.
#
# Refusing the preview outright would be the wrong fix: `validate` and `preview`
# are required to reach the SAME verdict for any rule — a preview is what an
# author believes about a rule before saving it — and validate never touches
# data. So the run still succeeds and the reply SAYS what it could not see.
SOURCE_TABLE = {
    "target": "targeting_perf",
    "keyword": "targeting_perf",
    "searchterm": "search_term_perf",
    "campaign": "campaign_perf",
    "adgroup": "targeting_perf",
    "product": "targeting_perf",
    "asin": "targeting_perf",
}


def evidence(conn, kind, window="CURRENT"):
    """{table, ok, reason} for the snapshot this entity kind reads.

    ROLLING windows read the true per-day tables and are already gated by
    `db.daily_window_gate` in the executor, so they are not judged here.
    """
    if window == "ROLLING":
        return None
    table = SOURCE_TABLE.get((kind or "").lower())
    if not table:
        return None
    if not _latest(conn, table):
        return {"table": table, "ok": False,
                "reason": f"{table} has no snapshots banked, so this rule was "
                          f"not evaluated against anything — that is not the "
                          f"same as nothing qualifying"}
    return {"table": table, "ok": True, "reason": None}
...
# different endpoints, and the DSL offers them as two entity kinds. They both
# live in targeting_perf, and `_load_targets` took `kind` and never used it, so
# `FOR EACH keyword` and `FOR EACH target` returned the identical set — 51,631
# rows in US, of which 49,901 are auto targeting expressions and only 1,712 are
# keywords. A rule authored for keywords was reaching every auto clause in the
# account. This is the same split the executor already routes on and that
# appctl.KEYWORD_MATCH_TYPES already names; the loader simply never applied it.
KEYWORD_MATCH_TYPES = ("BROAD", "EXACT", "PHRASE")


def _kind_filter(kind):
    """(sql_fragment, params) restricting targeting rows to one entity kind."""
    ph = ",".join("?" * len(KEYWORD_MATCH_TYPES))
    if kind == "keyword":
        return f"UPPER(COALESCE(match_type,'')) IN ({ph})", KEYWORD_MATCH_TYPES
    return f"UPPER(COALESCE(match_type,'')) NOT IN ({ph})", KEYWORD_MATCH_TYPES
...
def _load_targets(conn, kind, rolling=None):
    ...
    if rolling:
        ...
        where, kparams = _kind_filter(kind)
        sql = f"""SELECT MAX(target_id), campaign_id, ad_group_id, targeting, match_type,
                        SUM(impressions), SUM(clicks), SUM(cost),
                        SUM(orders), SUM(sales), 0, 0
                   FROM target_daily WHERE date BETWEEN ? AND ? AND {where}
                  GROUP BY campaign_id, ad_group_id, targeting, match_type"""
        params = tuple(rolling) + tuple(kparams)
    else:
        where, kparams = _kind_filter(kind)
        sql = f"""SELECT target_id, campaign_id, ad_group_id, targeting, match_type,
                        impressions, clicks, cost, orders, sales,
                        kenp_read, kenp_royalties
                   FROM targeting_perf WHERE date=? AND {where}"""
        params = (_latest(conn, "targeting_perf"),) + tuple(kparams)
    rows = []
