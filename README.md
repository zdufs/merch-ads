...
```bash
python3 list_profiles.py                       # find your advertising profile ids
ADS_MARKET=US python3 phase0_pull.py           # first data pull (slow — expect minutes)
ADS_MARKET=US python3 appctl.py metrics        # confirm you get real numbers back
```
...
- **Approval mode.** `python3 appctl.py approval-mode --on` makes the nightly run propose
...
- Your design catalog (`seasonal.json`) and your launch list (`tamas_launches.csv`) are
  gitignored operator data. The repo ships `.example` versions instead.
...
- The built-in campaign strategies (lottery, scavenger, TAMAS) encode **one** way of
  running Merch ads. Yours may differ. The **Rules DSL** exists so you can write your own
  logic without touching the engine.
- **TAMAS and organic-halo features are US-only.** They need the dated Merch
  `SALES_REPORT` export, which does not carry a marketplace column.
