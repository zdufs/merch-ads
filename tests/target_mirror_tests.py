...
class DslReadsMirror(unittest.TestCase):
    def test_target_bid_and_state_come_from_the_mirror(self):
        ...
        try:
            ...
            rows = {r.id: r for r in entities.load(conn, "keyword")}
            own = rows["t1"].fields
...
class AllTargetsTotal(unittest.TestCase):
    """`count` is the whole snapshot, `returned` is what the reply carries.

    They were the same number: `count` was computed after the slice, so it was
    always the cap. The Targets tab could say "top 2000 by spend" with no way to
    tell whether 2,001 or 50,822 targets sat behind it — the same conflation the
    accumulated-* commands were fixed for.
    """

    def _reply(self, path, limit):
        import contextlib
        import io
        import json
        import types
        ...
        try:
            with contextlib.redirect_stdout(buf):
                appctl.cmd_alltargets(types.SimpleNamespace(limit=limit))
        finally:
            db.DB_PATH = real
        return json.loads(buf.getvalue())["data"]

    def test_a_capped_reply_still_names_the_true_total(self):
        conn, path = temp_conn()
        try:
            seed_perf(conn)                      # two targets in the snapshot
            data = self._reply(path, limit=1)
            self.assertEqual(1, data["returned"])
            self.assertEqual(1, len(data["targets"]))
            self.assertTrue(data["truncated"])
            self.assertEqual(2, data["count"])   # the total, not the cap
        finally:
            conn.close(); os.unlink(path)

    def test_an_uncapped_reply_returns_everything_it_counted(self):
        conn, path = temp_conn()
        try:
            seed_perf(conn)
            data = self._reply(path, limit=10)
            self.assertFalse(data["truncated"])
            self.assertEqual(data["count"], data["returned"])
