...
class Executor(unittest.TestCase):
    ...
    def test_a_run_past_the_change_cap_is_refused_whole(self):
        """This used to assert TRUNCATION, and truncation was the bug.

        `execute` applied `changes[:cap]` and set `truncated: True`: half an
        account acted on, no refusal, and a flag that reached no screen. The cap
        now refuses the whole run — see tests/rules_volume_cap_tests.py for why
        the number is 500 and why a partial apply is worse than none.
        """
        conn, path = temp_conn()
        try:
            seed_fresh_snapshots(conn)
