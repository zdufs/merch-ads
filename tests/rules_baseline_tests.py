...
class Parsing(unittest.TestCase):
    ...
    def test_from_to_parses_to_a_range_window(self):
        expr = self._let_expr(
            "FOR EACH target:\n"
            "  LET b = target.acos IN FROM 8 DAYS AGO TO 60 DAYS AGO\n"
            "  target.note(\"x\")\n")
    ...
    def test_membership_in_a_list_is_not_a_window(self):
        prog = parse('FOR EACH target:\n'
                     '  IF target.match_type IN ["EXACT","PHRASE"]:\n'
                     '    target.note("x")\n')
...
TREND = ("FOR EACH target IN LAST 7 DAYS:\n"
         "  LET baseline = target.acos IN FROM 9 DAYS AGO TO 60 DAYS AGO\n"
         "  IF target.clicks >= 10 AND target.acos > baseline * 1.5:\n"
         "    target.pause()\n"
         "    target.note(\"recent {acos:percent} vs baseline\")\n")
