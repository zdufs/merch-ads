...
class Bank(unittest.TestCase):
    ...
    def test_each_bucket_carries_its_own_range(self):
        """The account-wide range says nothing about one currency series.

        US, UK and EU are three series of different lengths banked in one
        table. The Import screen read the top-level `months` — the union —
        and printed "60 months banked, continuous" on DE, whose own bucket
        was empty. A bucket has to be able to answer for itself.
        """
        old = csv_file([line(cur="USD", month=1)], name="History_backfill_2024.csv")
        both = csv_file([line(cur="USD", month=7), line(cur="EUR", month=7)],
                        name="History_backfill_2025.csv")
        history_import.bank(old, conn=self.conn)
        history_import.bank(both, conn=self.conn)
        cov = history_import.coverage(self.conn)
        by = {m["market"]: m for m in cov["by_market"]}
        self.assertEqual((cov["first_month"], cov["last_month"]), ("2024-01", "2025-07"))
        self.assertEqual((by["US"]["first_month"], by["US"]["last_month"]),
                         ("2024-01", "2025-07"))
        # The euro series holds ONE month, and it did not start in 2024.
        self.assertEqual((by["EU"]["first_month"], by["EU"]["last_month"]),
                         ("2025-07", "2025-07"))
        self.assertEqual(by["EU"]["months"], 1)
