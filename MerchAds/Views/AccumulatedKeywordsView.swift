...
struct AccumulatedKeywordsView: View {
    ...
    private var table: some View {
        Table(rows, selection: $selection, sortOrder: $sort.descendingFirst(), columnCustomization: $colPrefs) {
            TableColumn("Keyword / target", value: \.targeting) { r in
        ...
        .contextMenu(forSelectionType: AccumulatedKeywordRow.ID.self) { ids in
            ...
            Button("Negate \(label) everywhere (exact)", role: .destructive) {
                Task {
                    everywherePending = await resolveEverywhere(
                        appState, kind: "keyword", action: "negate", keys: keys,
                        verbLabel: "Negate", noun: "keyword", match: "exact")
                }
            }
            Button("Negate \(label) everywhere (phrase)", role: .destructive) {
                Task {
                    everywherePending = await resolveEverywhere(
                        appState, kind: "keyword", action: "negate", keys: keys,
                        verbLabel: "Negate", noun: "keyword", match: "phrase")
                }
            }
            ...
            Button("Pause \(label) everywhere", role: .destructive) {
                Task {
                    everywherePending = await resolveEverywhere(
                        appState, kind: "keyword", action: "pause", keys: keys,
                        verbLabel: "Pause", noun: "keyword")
                }
            }
        ...
        .alert("Set bid everywhere", isPresented: Binding(
            ...
            Button("Continue") {
                if let keys = bidEntryKeys, let bid = Double(bidText), bid > 0 {
                    ...
                    Task {
                        everywherePending = await resolveEverywhere(
                            appState, kind: "keyword", action: "setbid", keys: keys,
                            verbLabel: "Set bid", noun: "keyword", bid: bid)
                    }
                } else {
                    bidEntryKeys = nil
                }
                ...
                CSVDocument(
                    ...
                    rows: rows.map { r in
                        [r.targeting, r.matchType ?? "", String(r.campaigns), String(r.adGroups),
                         ...
                         r.acos.map { String($0) } ?? "", String(r.cvr)]
                    })
            }
        }
    }
    ...
    private func loadBreakdown() async {
        ...
        guard let targeting = primaryTargeting else { breakdown = []; breakdownLoading = false; return }
