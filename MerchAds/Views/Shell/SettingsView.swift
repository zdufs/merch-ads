...
struct SettingsView: View {
    ...
    private var appctlFound: Bool {
        PythonBridge.appctlURL(under: URL(fileURLWithPath: effectiveRoot)) != nil
    }
    ...
    @ViewBuilder private var sectionStack: some View {
        LazyVStack(alignment: .leading, spacing: Layout.Spacing.lg) {
            settingsSection(title: "Engine", subtitle: "local view-layer bridge") {
                ...
                Text("The repo folder: it holds the ads_data*.sqlite databases, with appctl.py in its engine/ subfolder. Secrets stay in the engine's .env — the app never reads them.")
                    .font(.caption)
            ...
            settingsSection(title: "Python", subtitle: "runtime resolution") {
                ...
                LabeledContent("Resolved python3",
                               value: pythonPath.isEmpty
                                   ...
                                   : pythonPath)
                Text("Leave empty to resolve python3 from your login shell — normally the same Homebrew python3 the nightly job runs.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
