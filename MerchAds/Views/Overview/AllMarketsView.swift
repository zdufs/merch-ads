...
struct AllMarketsView: View {
    ...
    @State private var ytdCoverage: [String: DailyCoverage] = [:]
    @State private var ytdCoverageFailures = Set<String>()
    ...
    /// ACOS per currency, never one number across all of them.
    ///
    /// This used to divide the sum of USD, GBP and EUR spend by the same mixed
    /// sum of sales. That ratio is not an ACOS in any currency: on live data it
    /// read 15.36% while the real figures were USD 14.09%, GBP 12.27% and EUR
    /// 17.73%. The two cards beside it already split by currency and this one
    /// claimed to be a "neutral local-currency rollup" while doing the opposite.
    /// There is no FX rate anywhere in the engine, so the only honest answer is
    /// one ACOS per currency.
    private var acosByCurrency: [(currency: String, acos: Double?)] {
        currencyTotals.map { t in
            (t.currency, t.sales > 0 ? t.spend / t.sales : nil)
        }
    }

    private var acosSummary: String {
        let parts = acosByCurrency.map { Format.percent($0.acos) }
        return parts.isEmpty ? "—" : parts.joined(separator: " · ")
    }

    /// The worst ACOS on the band, so the colour still means "look at this".
    private var worstAcos: Double? {
        acosByCurrency.compactMap(\.acos).max()
    }
    ...
    /// Rows are NOT all one line tall. The two YTD columns print a
    /// "Partial · since Jun 2026" caption under the figure whenever a market's
    /// year is short, and six of the seven markets carry one. A cap built on
    /// one height per row therefore came out short, and with the table's own
    /// scrolling switched off the shortfall ate the last row: on 2026-08-24 UK
    /// rendered as the top third of a row, sliced through the glyphs, with no
    /// scrollbar and no way to reach it.
    @ScaledMetric(relativeTo: .body) private var tableRowHeight: CGFloat = 34
    @ScaledMetric(relativeTo: .body) private var tableCaptionRowHeight: CGFloat = 50
    @ScaledMetric(relativeTo: .body) private var tableHeaderHeight: CGFloat = 40

    private var tableHeight: CGFloat {
        Self.tableHeight(rows: rows.count, captionRows: captionRowCount,
                         rowHeight: tableRowHeight,
                         captionRowHeight: tableCaptionRowHeight,
                         headerHeight: tableHeaderHeight)
    }

    /// Rows whose YTD cells carry the second line.
    private var captionRowCount: Int {
        rows.filter { $0.ytdStartLabel(fallbackFirstDay: ytdCoverage[$0.market]?.firstDay) != nil }
            .count
    }

    static func tableHeight(rows: Int, captionRows: Int, rowHeight: CGFloat,
                            captionRowHeight: CGFloat, headerHeight: CGFloat) -> CGFloat {
        let tall = min(max(captionRows, 0), max(rows, 0))
        return CGFloat(max(rows, 0) - tall) * rowHeight
            + CGFloat(tall) * captionRowHeight + headerHeight
    }
        ...
        Group {
            HStack(spacing: Layout.Spacing.sm) {
                ...
                StatCard(title: "Attributed sales", value: moneySummarySales,
                    ...
                    .mdCard()
                StatCard(title: "ACOS", value: acosSummary,
                         tint: AcosTier.select(acos: worstAcos).color,
                         symbol: "percent", subtitle: "per currency · never summed")
                    .mdCard()
    ...
    private func ytdValue(_ value: Double?, row: OverviewMarket) -> some View {
        VStack(alignment: .trailing, spacing: 1) {
            MoneyText(value: value, currency: row.currency)
            if let label = row.ytdStartLabel(
                fallbackFirstDay: ytdCoverage[row.market]?.firstDay) {
                Text(label)
                    .font(.caption2)
                    .foregroundStyle(label.hasPrefix("Partial")
                                     ? Theme.Colors.caution : Theme.Colors.muted)
            } else {
                Text(ytdCoverageFailures.contains(row.market)
                     ? "start unavailable" : "checking start…")
                    .font(.caption2)
                    .foregroundStyle(Theme.Colors.caution)
            }
        }
        .help(row.ytdBasis ?? "Year-to-date from banked coverage")
    }
    ...
    private func load() async {
        ...
        do {
            ...
            let response = try await bridge.call(
                OverviewResponse.self, ["overview", "--kind", kind])
            guard !Task.isCancelled else { return }
            overview = response
            // The engine sends ytd_partial / ytd_first_month now, so this
            // fallback only runs for a market that did NOT answer — a bundle
            // older than that change. Asking every market cost seven extra
            // bridge round trips on a screen that already has its answer.
            // Each market's own banked coverage, so a late-start year can never
            // inherit another market's.
            for row in response.markets where row.ytdFirstMonth == nil {
                do {
                    let monthly = try await bridge.call(
                        MonthlyResponse.self, ["monthly"], market: row.market)
                    guard !Task.isCancelled else { return }
                    if let coverage = monthly.coverage { ytdCoverage[row.market] = coverage }
                    else { ytdCoverageFailures.insert(row.market) }
                } catch is CancellationError {
                    return
                } catch {
                    guard !Task.isCancelled else { return }
                    ytdCoverageFailures.insert(row.market)
                }
            }
        } catch {
            guard !Task.isCancelled else { return }
