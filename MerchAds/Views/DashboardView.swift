...
struct DashboardView: View {
    ...
    private var navigationSubtitle: String {
        if let asOf = snapshot?.metrics.dataAsOf {
            return "\(appState.selectedMarket) · data through \(Format.euDate(asOf))"
        }
    ...
    private var alertStrip: some View {
        ...
        let killCount = snapshot?.killList.value?.count
        return VStack(alignment: .leading, spacing: Layout.Spacing.xs) {
            HStack(spacing: Layout.Spacing.xs) {
                ...
                AlertPill(
                    text: killCount.map { "\($0) kill candidate\($0 == 1 ? "" : "s")" }
                        ?? unavailableLabel(snapshot?.killList),
                    symbol: "xmark.bin.fill",
                    tint: (killCount ?? 0) > 0 ? Theme.Colors.critical : .secondary,
