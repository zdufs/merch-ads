...
struct ApprovalsView: View {
    ...
    private var header: some View {
        FilterBar {
            ...
            .disabled(approvedCount == 0 || applying || appState.killActive)
            .help("Writes the checked items to Amazon: negatives are permanent, pauses can be undone from the Audit Trail")
            .confirmationDialog(
                ...
                Text("Apply to \(intent.scope.confirmationDescription). Negatives are permanent; pauses remain undoable from Audit Trail.")
            }
        ...
        return VStack(spacing: 0) {
            ...
            Table(rows, selection: $pauseSelection, sortOrder: $pauseSort.descendingFirst(),
                ...
                TableColumn("ASIN", value: \.asinValue) { pause in
                    Text(pause.asin ?? "—").font(.body.monospaced())
                }
    ...
    private func apply(_ intent: ActionIntent) async {
        ...
        do {
            ...
            guard case .negativesApply(let negativesApplied, let pausesApplied) = receipt.result else { return }
