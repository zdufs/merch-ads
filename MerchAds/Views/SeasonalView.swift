...
struct SeasonalView: View {
    ...
    var body: some View {
        ...
        .confirmationDialog(
            ...
            Text("Pause \(preview?.pause.count ?? 0) and re-enable \(preview?.enable.count ?? 0) ad group(s) in \(intent.scope.confirmationDescription). Reversible from the Audit Trail.")
        }
    }
