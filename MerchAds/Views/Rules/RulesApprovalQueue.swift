...
struct RulesApprovalQueue: View {
    ...
    private var content: some View {
        if loading && changes.isEmpty {
            ...
            // Greedy, like the ProgressView above. Without this the branch
            // takes only its intrinsic height and the detail column centres
            // the WHOLE screen — title, picker and toolbar floating in the
            // middle of the window under 400pt of blank space, which reads as
            // a rendering failure rather than an empty queue.
    ...
    private func apply() async {
        ...
        do {
            ...
            if case .rulesApprove(let count, let blocked, let heldBack, let stale,
                                  let notApplied, let note) = receipt.result {
                if let blocked {
                    status = "Blocked: \(blocked)"
                } else {
                    var parts = ["Applied \(count)"]
                    if heldBack > 0 {
                        parts.append("\(heldBack) held back — another rule had already claimed that entity")
                    }
                    // A proposal whose rule was edited, disabled or deleted after
                    // it was queued would write a value no rule now proposes.
                    if stale > 0 {
                        parts.append("\(stale) skipped — the rule changed after this was queued. Re-collect to refresh them.")
                    }
                    if notApplied > 0 {
                        parts.append(note ?? "\(notApplied) rule writes were not applied")
                    } else if let note {
                        parts.append(note)
                    }
