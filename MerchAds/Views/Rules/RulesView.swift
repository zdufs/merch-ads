...
struct RulesView: View {
    ...
    /// My Rules lists only ACTIVE (enabled) rules — the ones actually running.
    /// Every rule, on or off, lives in the Library, which is where you turn one on
    /// or off.
    private var activeRules: [RuleSummary] { rules.filter(\.enabled) }
    ...
    private var listPane: some View {
        VStack(spacing: 0) {
            ...
            .onChange(of: selection) { old, name in
                ...
                if let name { Task { await loadRule(name) } }
            }
    ...
    private func loadList() async {
        ...
        if let r = try? await bridge.call(RuleListResponse.self, ["rules-list"], market: appState.selectedMarket) {
            rules = r.rules
        }
    }

    private func loadRule(_ name: String) async {
        guard let bridge = try? appState.makeBridge() else { return }
        if let r = try? await bridge.call(Rule.self, ["rules-get", "--rule", name], market: appState.selectedMarket) {
    ...
    private func apply() async {
        await withBridge { bridge in
            ...
            _ = try await bridge.call(Rule.self, ["rules-save"], market: appState.selectedMarket,
                                      stdin: try JSONSerialization.data(withJSONObject: payload),
                                      preferWorker: false)
            let r = try await bridge.call(RuleRunResult.self, ["rules-run", "--apply", "--rule", draft.name],
                                          market: appState.selectedMarket, preferWorker: false)
            status = r.blocked != nil ? "Blocked: \(r.blocked!)" : "Applied \(r.count ?? 0) change(s)"
            await loadList()
        }
    }
...
private struct RuleRunResult: Codable {
    let applied: Bool?
    let count: Int?
    let blocked: String?
    let matched: Int?
