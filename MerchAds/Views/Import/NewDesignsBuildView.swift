...
struct NewDesignsBuildView: View {
    ...
    private static let howToSteps = [
        "Amazon Merch on Demand → MerchFlow → Products.",
        ...
        return Group {
            HStack(spacing: Layout.Spacing.sm) {
                ...
                StatCard(title: "Approved", value: preview == nil ? "—" : Format.count(approved.count),
                    ...
                    .mdCard()
                StatCard(title: "Markets", value: Format.count(appState.markets.filter(\.hasData).count),
                         symbol: "globe")
                    .mdCard()
            }
        }
    ...
    private var dropTarget: some View {
        VStack(spacing: Layout.Spacing.sm) {
            ...
            Text("export_products_….csv — the full catalog file from Merch on Demand.")
                .font(.callout)
                .foregroundStyle(.secondary)
            ...
            Table(route.designs, selection: $rowSel) {
                ...
                TableColumn("ASIN") { design in
                    VStack(alignment: .leading, spacing: Layout.Spacing.xxs) {
                        Text(design.asin).font(.body.monospaced())
    ...
    private func requestBuildAllMarkets() {
        ...
        let codes = appState.markets.filter(\.hasData).map(\.code)
        ...
        do {
            ...
            do {
                ...
                if let adopted, adopted.movedToPod {
                    // the file moved into the engine folder — keep our reference valid
                ...
                IssueCenter.report(source: .adopt, title: "Export adoption failed",
                                   ...
                                   fix: .operatorCommand("ADS_MARKET=US python3 map_products.py"))
            }
            allResults = results
        } catch {
    ...
    private func adoptSummary(_ export: AdoptedExport) -> String {
        ...
        parts.append(export.movedToPod
            ? "\(export.adopted) was moved into the engine folder as the new canonical catalog (the nightly job reads it from there)."
