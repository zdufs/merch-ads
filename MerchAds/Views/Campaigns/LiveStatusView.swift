...
struct LiveStatusView: View {
    ...
    @State private var activeRequest: LiveStatusRequestToken?
    ...
    var body: some View {
        ...
        .onChange(of: query) {
            if !activeRequestMatchesCurrentInput { cancelRequest() }
            clearLookupResults()
        }
        .onChange(of: appState.selectedMarket) {
            cancelRequest()
            clearLookupResults()
            if !query.isEmpty { startCachedLookup() }
        }
        ...
        FilterBar {
            ...
            Toggle("All markets", isOn: $allMarkets)
                ...
                .onChange(of: allMarkets) {
                    if !trimmedQuery.isEmpty { startCachedLookup() }
                }
        }
    }
    ...
    private func lookupAllMarkets(query: String, token: LiveStatusRequestToken) async {
        guard !query.isEmpty, isCurrent(token) else { return }
        isLoadingAll = true
        defer { if isCurrent(token) { isLoadingAll = false } }
        marketResults = []
        failedMarkets = []
        guard let bridge = try? appState.makeBridge() else {
            if isCurrent(token) { failedMarkets = ["all — engine bridge unavailable"] }
            return
        }
        for market in appState.markets.filter(\.hasData).map(\.code) {
            do {
                let response = try await bridge.call(AsinResponse.self, ["asin", query],
                                                     market: market)
    ...
    private func startCachedLookup() {
        let token = beginRequest()
        guard !token.query.isEmpty else { return }
        requestTask = Task { await lookupCached(token) }
    }

    private func startLiveRefresh() {
        let token = beginRequest()
        guard !token.query.isEmpty else { return }
        requestTask = Task { await refreshLive(token) }
    }

    private func beginRequest() -> LiveStatusRequestToken {
        requestTask?.cancel()
        requestGeneration += 1
        let token = LiveStatusRequestToken(generation: requestGeneration,
                                           market: appState.selectedMarket,
                                           query: trimmedQuery, allMarkets: allMarkets)
        activeRequest = token
        return token
    }
    ...
    private func clearLookupResults() {
        cached = nil
        cachedError = nil
        selection = []
        liveStates = [:]
    ...
    private var activeRequestMatchesCurrentInput: Bool {
        activeRequest?.matches(generation: requestGeneration,
                               market: appState.selectedMarket,
                               query: trimmedQuery, allMarkets: allMarkets) == true
    }

    private func isCurrent(_ token: LiveStatusRequestToken) -> Bool {
        !Task.isCancelled
            && token.matches(generation: requestGeneration,
                             market: appState.selectedMarket,
                             query: trimmedQuery, allMarkets: allMarkets)
    }

    private func lookupCached(_ token: LiveStatusRequestToken) async {
        guard isCurrent(token) else { return }
        if token.allMarkets {
            await lookupAllMarkets(query: token.query, token: token)
            return
        }
        isLoadingCached = true
        defer { if isCurrent(token) { isLoadingCached = false } }
        cachedError = nil
        liveStates = [:]
        liveFetchedAt = nil
        liveError = nil
        do {
            ...
            let response = try await bridge.call(AsinResponse.self, ["asin", token.query],
                                                 market: token.market)
            guard isCurrent(token) else { return }
            cached = response
        } catch {
    ...
    private func refreshAfterAction(_ intent: ActionIntent) async {
        ...
        if hadLiveState { startLiveRefresh() } else { startCachedLookup() }
    }

    private func refreshLive(_ token: LiveStatusRequestToken) async {
        guard isCurrent(token), !token.query.isEmpty, !token.allMarkets else { return }
        if cached == nil {
            do {
                let bridge = try appState.makeBridge()
                let response = try await bridge.call(AsinResponse.self, ["asin", token.query],
        ...
        isLoadingLive = true
        defer { if isCurrent(token) { isLoadingLive = false } }
        liveError = nil
        do {
            let bridge = try appState.makeBridge()
            let response = try await bridge.call(LiveStateResponse.self,
                                                 ["livestate", token.query],
                                                 market: token.market)
            guard isCurrent(token) else { return }
...
struct LiveStatusRequestToken: Equatable {
    let generation: Int
    let market: String
    let query: String
    let allMarkets: Bool

    func matches(generation: Int, market: String, query: String,
                 allMarkets: Bool) -> Bool {
        self.generation == generation && self.market == market
            && self.query == query && self.allMarkets == allMarkets
    }
}
