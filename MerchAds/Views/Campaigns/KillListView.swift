...
struct KillListView: View {
    ...
    private static let sortFields: [String: KeyPathComparator<KillDesign>] = [
        ...
        "clicks": .init(\.clicks), "orders": .init(\.orders),
        // \.cvrValue, matching the column. The descriptor and the column must
        // be the same key path or SortPrefs cannot match them, and the saved
        // sort silently reverts on the next launch.
        "cvr": .init(\.cvrValue),
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if mode == .bleeding { economicsUnavailableBanner }
            if mode == .bleeding { sparedBanner }
            if mode == .bleeding { skippedBanner }
            Divider()
            ...
            if isLoading && killList == nil && stale == nil {
                ...
                if !(killList?.designs.isEmpty ?? true) && !filterText.isEmpty {
                    ...
                    ContentUnavailableView {
                        Label(emptyStateTitle,
                              systemImage: economicsUnavailable
                                  ? "exclamationmark.triangle" : "checkmark.seal")
                    } description: {
                        // Only say "no design" when every design was actually
                        // judged. Otherwise this is a claim about designs the
                ...
                Table(designs, selection: $selection, sortOrder: $sortOrder.descendingFirst(),
                    ...
                    TableColumn("CVR", value: \.cvrValue) { design in
                ...
                .copyableRows(designs, primaryLabel: "ASIN",
                              ...
                              row: { "\($0.asin ?? "")\t\($0.state ?? "")\t\($0.clicks)\t\($0.orders)\t\($0.cvr.map { String($0) } ?? "")\t\($0.spend)\t\($0.sales)\t\($0.acos.map { String($0) } ?? "")\t\($0.breakEven.map { String($0) } ?? "")" })
    ...
    /// How many designs never reached a verdict, and why.
    private var unjudged: Int { killList?.skipped?.unjudged ?? 0 }

    /// True when the engine could not judge ANY design in this market.
    private var economicsUnavailable: Bool { killList?.economicsUnavailable ?? false }

    /// The empty list means three different things, and the heading has to pick
    /// the right one. "Nothing to kill" is a claim about every design, and it is
    /// only true when every design was actually measured.
    private var emptyStateTitle: String {
        if economicsUnavailable { return "Nothing could be judged" }
        return unjudged > 0 ? "Nothing flagged" : "Nothing to kill"
    }

    private var emptyStateMessage: String {
        if economicsUnavailable {
            return "\(appState.selectedMarket) has no economics tables yet, so no "
                 + "design was measured against a break-even. This is not the same "
                 + "as nothing bleeding — see above."
        }
        if unjudged > 0 {
            return "No design in \(appState.selectedMarket) that could be judged is "
                 + "below the CVR floor while over break-even. \(unjudged) could not "
                 + "be judged — see above."
        }
        return "No design in \(appState.selectedMarket) is below the CVR floor "
             + "while over break-even."
    }

    /// The kill list needs this market's economics tables. Without them the
    /// engine returns an empty list AND says so in `econ` — and an empty list
    /// on its own is indistinguishable from a healthy market with nothing
    /// worth killing. This is `skippedBanner`'s failure one level up: there
    /// some designs could not be judged, here none could, and the reassuring
    /// version of that screen is the one that used to show.
    ///
    /// It fires on a database that has never been pulled or mapped, which is
    /// precisely a new install — so this was the first thing a new operator
    /// would have been told wrongly.
    @ViewBuilder
    private var economicsUnavailableBanner: some View {
        if economicsUnavailable {
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                Image(systemName: "exclamationmark.triangle.fill")
                    .foregroundStyle(Theme.Colors.critical)
                VStack(alignment: .leading, spacing: 2) {
                    Text("No design in \(appState.selectedMarket) could be judged")
                        .font(.callout.weight(.medium))
                    Text("This market's economics tables are not built yet, so no "
                         + "break-even exists to measure against. Run a pull and a "
                         + "product map once, then reopen this screen. "
                         + "An empty list here does not mean nothing is bleeding.")
                        .font(.caption)
    ...
    /// The kill list judges a design against its OWN break-even, so a design
    /// with no usable price is excluded BEFORE any threshold runs. Counting
    /// those on screen is the difference between "nothing qualified" and
    /// "nothing we could look at qualified" — and the screen states a negative,
    /// so the distinction is the whole meaning of the sentence.
    @ViewBuilder
    private var skippedBanner: some View {
        if let skipped = killList?.skipped, skipped.unjudged > 0 {
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                Image(systemName: "questionmark.circle.fill")
                    .foregroundStyle(Theme.Colors.caution)
                VStack(alignment: .leading, spacing: 2) {
                    Text("\(skipped.unjudged) design\(skipped.unjudged == 1 ? "" : "s") could not be judged")
                        .font(.callout.weight(.medium))
                    Text(skipped.reasons.joined(separator: " · "))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                Spacer()
            ...
            .padding(Layout.Spacing.md)
            .background(Theme.Colors.caution.opacity(0.08),
                        in: RoundedRectangle(cornerRadius: Layout.Radius.medium))
            .padding(.horizontal, Layout.Spacing.sm)
            .padding(.bottom, 4)
            .help("A kill verdict needs the design's own break-even ACOS. These designs have no usable price yet, so no threshold was applied to them.")
        }
    }
                    ...
                    CSVDocument(
                        ...
                        rows: killList.designs.map { design in
                            [design.asin ?? "", design.type ?? "", design.state ?? "",
                             String(design.clicks), String(design.orders),
                             design.cvr.map { String($0) } ?? "",
