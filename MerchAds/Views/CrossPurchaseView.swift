...
struct CrossPurchaseView: View {
    ...
    private var designsTable: some View {
        Table(designRows, selection: $designSel) {
            TableColumn("Design") { Text($0.adGroup ?? $0.advertisedAsin ?? "—") }
            TableColumn("ASIN") { Text($0.advertisedAsin ?? "—").monospaced() }
    ...
    private var pairsTable: some View {
        Table(pairRows, selection: $pairSel) {
            TableColumn("Ad shown for") { Text($0.advertisedAsin ?? "—").monospaced() }
            TableColumn("Design") { Text($0.adGroup ?? "—") }
            TableColumn("Bought instead") { Text($0.purchasedAsin ?? "—").monospaced() }
            TableColumn("Sales") {
