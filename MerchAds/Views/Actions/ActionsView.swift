...
struct ActionsView: View {
    ...
    /// The reload runs on every market change and the plan is scoped to the
    /// market it was previewed in, so Apply has to be shut while it is in
    /// flight — it used to stay live holding the previous market's plan.
        ...
        groupedSection(title: "Reset inflated bids", subtitle: appState.selectedMarket) {
            HStack(spacing: Layout.Spacing.sm) {
                ...
                Button("Apply") { pendingReset = resetIntent }
                    .disabled(busy || resetLoading || (resetPreview?.count ?? 0) == 0)
            }
        }
        .confirmationDialog(
            ...
            presenting: pendingReset) { intent in
            Button("Reset bids", role: .destructive) {
                Task { await applyReset(intent) }
            }
        } message: { intent in
            // The market the PLAN names, not whichever one the picker is on.
            // The dialog used to read the picker, so it could promise DE while
            // the intent still carried the US plan behind it.
            Text("Restore original bids minus 10% in \(intent.scope.confirmationDescription). Each write is logged and undoable.")
        }
    }
    ...
    private func loadResetPreview() async {
        actionError = nil
        // Drop the previous market's plan BEFORE the await. Leaving it on
        // screen under the new market's header is what let Apply fire a plan
        // scoped to the market the operator had just switched away from.
        let market = appState.selectedMarket
        resetPreview = nil
        resetIntent = nil
        resetPreviewReceipt = nil
        pendingReset = nil
        resetLoading = true
        ...
        do {
            ...
            guard market == appState.selectedMarket, !Task.isCancelled else { return }
    ...
    private func applyReset(_ intent: ActionIntent) async {
        // The picker can move between the preview and the confirmation. This
        // plan writes to the market it was previewed in, so a mismatch is
        // refused rather than applied under a name the dialog never said.
        ...
        do {
            ...
            guard !receipt.rehearsed else { return }
            guard case .resetBids(let count, let reduction, let partialNote) = receipt.result else { return }
    ...
    private func runBackfill() async {
        ...
        do {
            ...
            let r = try await bridge.call(BackfillResponse.self, ["backfill-daily"],
                                          market: appState.selectedMarket,
                                          preferWorker: false)
            backfillResult = r.code == 0
                ? "Backfill finished — daily history re-banked."
                : "Backfill exited \(r.code) — tail: \(r.failureTail)"
            await appState.refresh()
        } catch {
            actionError = error.localizedDescription
        }
