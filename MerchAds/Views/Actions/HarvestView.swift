...
struct HarvestView: View {
    ...
    private func applyPrune(_ intent: ActionIntent) async {
        ...
        do {
            ...
            guard !receipt.rehearsed else { return }
            guard case .harvestPruneApply(let paused, let shortfall) = receipt.result else {
                throw ActionResultError.unexpectedResponse
            }
            // Amazon refusing the whole approved batch also pauses 0, and that
            // read as a green "Paused 0 keywords." The shortfall belongs in the
            // error bar, where a number short of what was asked for belongs.
            if let shortfall {
                pruneResult = nil
                pruneApplyError = shortfall
            } else {
    ...
    private func promote(_ intent: ActionIntent) async {
        ...
        do {
            ...
            let outcome = PromotionApplySummary(keywordExit: keywordExit, asinExit: asinExit)
            if let failure = outcome.partialFailureMessage {
                promoteOutput = nil
                promoteError = failure
            } else {
                promoteOutput = outcome.successMessage
            }
            await load()
        } catch {
            promoteError = error.localizedDescription
...
struct PromotionApplySummary: Equatable {
    let keywordExit: Int?
    let asinExit: Int?

    var partialFailureMessage: String? {
        var failures: [String] = []
        if let keywordExit, keywordExit != 0 { failures.append("keyword phase exited \(keywordExit)") }
        if let asinExit, asinExit != 0 { failures.append("ASIN phase exited \(asinExit)") }
        return failures.isEmpty ? nil
            : "Promotion partial failure: " + failures.joined(separator: " · ")
    }
    ...
    var successMessage: String {
        ...
        return parts.isEmpty ? "nothing to promote" : parts.joined(separator: " · ")
    }
}
