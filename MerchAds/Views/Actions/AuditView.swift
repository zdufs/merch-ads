...
struct AuditView: View {
    ...
    private var undoableCount: Int {
        (audit?.writes ?? []).filter { $0.undoable && !undoneRows.contains(undoKey($0.rowId)) }.count
    }
        ...
        Group {
            HStack(spacing: Layout.Spacing.sm) {
                StatCard(title: "Writes today", value: Format.count(writesToday),
                         symbol: "clock.fill",
                         subtitle: noOpsToday > 0
                            ? "+ \(noOpsToday) no-op row\(noOpsToday == 1 ? "" : "s")" : nil)
                    .mdCard()
