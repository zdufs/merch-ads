...
struct SeasonalView: View {
    ...
    /// Advertised designs first, then by season.
    ///
    /// The tag map is the whole catalogue — 15,674 US designs — but only the few
    /// hundred with an ad group can be paused or released, and those are the
    /// only rows the scheduler will ever act on. Sorting by season put the
    /// un-advertised 96% at the top, so the screen opened on fifteen thousand
    /// rows of "—" and the rows that move money were unreachable by scrolling.
    @State private var tagSort = SortPrefs.load(
        TableID.seasonalTags, fields: tagSortFields,
        fallback: [KeyPathComparator(\SeasonTag.adGroups, order: .reverse),
                   KeyPathComparator(\SeasonTag.label)])

    /// Hide designs with no ad group. Off by default: the full map is still what
    /// proves a tag exists at all, which is how the 2026-08-15 tag loss was found.
    @State private var advertisedOnly = false
    ...
    private var taggedPane: some View {
        VStack(spacing: 0) {
            ...
            if let tags = seasons?.tags, !tags.isEmpty {
                taggedTable(advertisedOnly ? tags.filter { $0.adGroups > 0 } : tags)
            } else {
                ContentUnavailableView {
    ...
    private var tagBar: some View {
        FilterBar {
            ...
            Toggle("Advertised only", isOn: $advertisedOnly)
                .toggleStyle(.checkbox)
                .help("Show only tagged designs that have an ad group — the ones the scheduler can pause or release")
            if let tags = seasons?.tags, !tags.isEmpty {
                let advertised = tags.count { $0.adGroups > 0 }
                Text("\(Format.count(advertised)) of \(Format.count(tags.count)) advertised")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        } trailing: {
    ...
    private func apply(_ intent: ActionIntent) async {
        ...
        do {
            let receipt = try await appState.actionCoordinator.execute(
                intent, context: appState.actionPolicyContext, confirmed: true)
            guard !receipt.rehearsed else { return }
            guard case .seasonalApply(let paused, let enabled, let note) = receipt.result else { return }
            if let note {
                lastResult = nil
                actionError = "\(note) Applied: paused \(paused), re-enabled \(enabled)."
            } else {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Table(rows, selection: $selection, sortOrder: $sortOrder.descendingFirst()) {
                ...
                TableColumn("ASIN", value: \.asin) { AsinLink(asin: $0.asin) }
                    .width(min: 90, ideal: 110)
