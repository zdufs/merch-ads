...
///
/// Scope: EVERY advertised design. It began scoped to a retired strategy whose
/// campaigns held one ASIN — a shape that cannot describe a lottery campaign
/// holding a thousand. The unit is now the DESIGN, and its ad facts are
/// summed across every ad group advertising it, so lottery, scavenger, standard
/// and harvested designs all appear. The "Campaigns" column says which.
...
struct HaloView: View {
            ...
            ExportButton(filename: "organic-halo-\(appState.selectedMarket)") {
                CSVDocument(
                    headers: ["asin", "design", "ad_start", "ad_spend", "ad_clicks",
                              "base_per_day", "post_per_day", "halo_est", "net_halo",
                              "traz_window", "flags"],
                    rows: rows.map { d in
                        [d.asin, d.title ?? d.label, d.adStart ?? "", String(d.adSpend), String(d.adClicks),
                         String(d.baseRate), String(d.postRate), String(d.haloEst), String(d.netHalo),
        ...
        Table(rows, selection: $selection, sortOrder: $sort.descendingFirst(), columnCustomization: $colPrefs) {
            TableColumn("Design", value: \.label) { d in
                HStack(spacing: Layout.Spacing.xs) {
                    AsinLink(asin: d.asin, text: d.label, font: .body)
            ...
            TableColumn("Campaigns", value: \.campaignTypesValue) { d in
                Text(d.campaignTypes ?? "—")
                    .foregroundStyle(Theme.Colors.muted)
                    .lineLimit(1).truncationMode(.tail)
                    .help("Which campaign kinds advertise this design")
            }
            .width(min: 80, ideal: 130)
            .customizationID("campaignTypes")
            ...
            TableColumn("Net halo", value: \.netHalo) { d in
                MoneyText(value: d.netHalo, currency: currency, color: haloColor(d))
            }
            .width(min: 52, ideal: 78)
            .customizationID("net")
    ...
    private func flagHelp(_ flags: String) -> String {
        ...
        if flags.contains("control") {
            parts.append("Control: this design was never advertised, so there is no halo to estimate here. Use it as a reference — each advertised design is measured against its own pre-ad baseline, and this row shows what un-advertised royalty did over the same window.")
        }
    ...
    private func load() async {
        ...
        do {
            ...
            let fresh = try await bridge.call(HaloResponse.self, ["halo", "--limit", "0"],
