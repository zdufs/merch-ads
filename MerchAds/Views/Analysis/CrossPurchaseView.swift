...
struct CrossPurchaseView: View {
    ...
    /// Rows that MEASURED something. 28 of the 51 US pairs had zero sales,
    /// zero units and zero purchases, so nothing was bought on them — and the
    /// screen headlined all 51 as "clicked this, bought that", overstating the
    /// measured halo by more than double. The screen's own "Designs earning
    /// halo" card already counted the honest way and disagreed with the
    /// section header beside it by roughly a factor of two (2026-08-24).
    private var convertedPairs: Int { Self.convertedPairs(pairRows) }
    private var sellingDesigns: Int { designRows.filter { $0.otherSales > 0 }.count }

    static func convertedPairs(_ pairs: [CrossPurchasePair]) -> Int {
        pairs.filter { $0.units > 0 || $0.sales > 0 || $0.purchases > 0 }.count
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            SectionHeader(title: "Designs that sell other designs",
                          subtitle: "sent sales elsewhere · \(designRows.count) observed in total",
                          count: sellingDesigns)
                .padding(.horizontal, Layout.Spacing.sm)
                .padding(.top, Layout.Spacing.xs)
            designsTable
            Divider()
            SectionHeader(title: "Clicked this, bought that",
                          subtitle: "pairs that measured a sale · \(pairRows.count) observed in total",
                          count: convertedPairs)
                .padding(.horizontal, Layout.Spacing.sm)
                .padding(.top, Layout.Spacing.xs)
            pairsTable
        }
    ...
    private var headerBand: some View {
        ...
        return HStack(alignment: .top, spacing: Layout.Spacing.sm) {
            ...
            StatCard(title: "Cross-sell pairs",
                     value: "\(convertedPairs)",
                     symbol: "arrow.left.arrow.right",
                     subtitle: convertedPairs == pairRows.count
                        ? "clicked this, bought that"
                        : "bought · \(pairRows.count - convertedPairs) more observed with no sale")
                .frame(maxHeight: .infinity, alignment: .topLeading)
                .mdCard()
            StatCard(title: "Designs earning halo",
                     value: "\(sellingDesigns)",
                     symbol: "tshirt",
