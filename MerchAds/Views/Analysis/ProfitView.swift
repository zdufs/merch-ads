...
struct ProfitView: View {
        ...
        return Group {
            HStack(spacing: Layout.Spacing.sm) {
                ...
                StatCard(title: "Ad spend",
                         value: spendCardValue,
                         symbol: "creditcard.fill",
                         subtitle: spendCardCaption)
                    .mdCard()
    ...
    /// Ad spend needs no royalty, so it is shown whenever the reply arrived.
    ///
    /// On KDP US all four cards read "—" with "no royalty data banked for this
    /// market" underneath, this one included — while the Dashboard on the same
    /// profile showed 3,11 US$ of spend for the same window. A card blanked
    /// for a reason that does not apply to it teaches the reader to discount
    /// the reason on the cards where it is true.
    private var spendCardValue: String {
        guard profit != nil else { return "—" }
        return Format.money(profit?.totalSpend ?? 0, currency: currency)
    }

    /// Says why covered spend can be zero on a market that plainly spent money:
    /// every ad group is a multi-ASIN cohort, which belongs to no one design.
    private var spendCardCaption: String {
        let base = "covered spend · cohort spend excluded"
        let cohort = profit?.unattributedCohortSpend ?? 0
        guard profit != nil, (profit?.totalSpend ?? 0) == 0, cohort > 0 else { return base }
        let groups = profit?.unattributedCohortGroups ?? 0
        return "covered spend · \(Format.money(cohort, currency: currency)) sits in "
            + "\(groups) multi-ASIN group\(groups == 1 ? "" : "s") and belongs to no one design"
    }
        ...
        HStack(spacing: Layout.Spacing.sm) {
            ...
            if let listed = profit?.designs, !listed.isEmpty {
                ExportButton(filename: "profit-\(appState.selectedMarket)") {
                    CSVDocument(
                        headers: ["asin", "ad_group_id", "type", "orders", "spend", "sales",
                                  "royalty_est", "profit", "royalty_roi"],
                        rows: listed.map { design in
                            [design.asin ?? "", design.adGroupId,
                             design.type ?? "", String(design.orders),
    ...
    /// The section's unit is the AD GROUP, and the header has to say so.
    ///
    /// The engine returns one row per (asin, ad_group_id). Sorted to the
    /// extremes, the same ASIN lands near the top AND near the bottom: on US,
    /// ten designs showed a profitable row and a loss-making row at once, one
    /// saying "bid up" and the other "bid down".
    private var designsSubtitle: String {
        let base = "profit extremes · one row per ad group"
        let repeated = Self.repeatedAsins(designs)
        guard repeated > 0 else { return base }
        return base + " · \(repeated) design\(repeated == 1 ? "" : "s") appear\(repeated == 1 ? "s" : "") more than once"
    }

    /// How many ASINs hold more than one of the listed rows.
    static func repeatedAsins(_ rows: [ProfitDesign]) -> Int {
        var seen: [String: Int] = [:]
        for row in rows {
            guard let asin = row.asin, !asin.isEmpty else { continue }
            seen[asin, default: 0] += 1
        }
    ...
    private var designsTable: some View {
        VStack(spacing: 0) {
            ...
            Table(designs, selection: $designSelection, sortOrder: $designSort.descendingFirst(), columnCustomization: $designColPrefs) {
                ...
                // The row's REAL unit. One design can be advertised by several
                // ad groups, and ten US designs appeared here as both a
                // loss-maker and a profit-maker with opposite advice, with
                // nothing on the row to tell the two apart (2026-08-24).
                TableColumn("Ad group", value: \.adGroupId) { design in
                    Text(design.adGroupId)
