...
enum IssueDerivation {
    ...
    static func live(health: HealthResponse?, econGate: EconGateResponse?,
        ...
        // An OPEN gate says the prices on hand are FRESH. It says nothing about
        // how many designs the gate can judge at all, and those two questions
        // share one green light.
        //
        // Ask the gate itself, not the catalogue. Catalogue coverage counts
        // designs with no list price, and most of them do not need one: only a
        // US standard tee is priced per design. Warning on that number said a
        // quarter of the account was unmanaged when the true figure was 182 —
        // an alarm that wrong gets muted, and then the real one is missed too.
        if let cov = econGate?.econCoverage, let actionable = cov.actionable,
           actionable > 0 {
            // Count PRODUCTS in the headline — one product can be advertised by
            // several ad groups, and the ad-group figure reads bigger than the
            // thing the operator would go and fix.
            //
            // And headline only the LIVE ones. A removed listing has no price
            // and can never get one, so a re-export cannot help it: on
            // 2026-08-22 that was 165 of 174, and the fix command was an errand
            // that could not succeed. Say the removed ones separately, and only
            // as information.
            let products = cov.actionableAsins ?? actionable
            let live = cov.actionableLive ?? products
            let removed = cov.actionableRemoved ?? 0
            let spend = cov.actionableSpend.map {
                " They spent \(Format.money($0, currency: econGate?.currency)) in the last 30 days."
            } ?? ""
            let stillOn = cov.actionableRemovedEnabled ?? 0
            let removedNote = removed > 0
                ? " A further \(removed.formatted()) are listings Amazon no longer sells (\(Self.removedSummary(cov.removedStatuses))), so no export will ever price them"
                  + (stillOn > 0
                     ? " — \(stillOn.formatted()) still have an ENABLED ad group; pause those if they are not earning."
                     : ", and their ads are already paused.")
                : ""
            let headline = live > 0
                ? "\(live.formatted()) products have no usable economics"
                : "\(removed.formatted()) advertised products are no longer for sale"
            out.append(.live(.warning, .econGate, key: "econCoverage",
                             title: headline,
                             detail: "Their break-even cannot be worked out, so bids, pauses and negatives skip them rather than guess — they are not paused, not flagged and not counted.\(spend) Across \(actionable.formatted()) ad groups.\(removedNote)",
                             market: econGate?.market,
                             fix: live > 0
                                ? .operatorCommand("ADS_MARKET=US python3 engine/map_products.py")
                                : nil,
                             at: now))
        }

        // Freshness is per DESIGN, not per catalogue: a chunk exported months
        // ago sits beside one exported today, and the gate reads the newest.
        // Matters for US tees, which price per design; other types read the
        // type table and are unaffected by an old chunk.
        if let cov = econGate?.catalog, let stale = cov.pricesOlderThanGate, stale > 0 {
            out.append(.live(.warning, .econGate, key: "catalogStalePrices",
                             title: "\(stale.formatted()) designs priced from an out-of-date export",
                             detail: "Their list price came from a chunk older than the freshness gate allows, but the newest chunk is fresh, so the gate stays open. This changes US tee economics; other product types price from the type table.",
                             market: econGate?.market,
                             fix: .operatorCommand("ADS_MARKET=US python3 engine/map_products.py"),
                             at: now))
        }
        ...
        for market in health?.markets ?? [] {
            ...
            if market.hasData, let latest = market.latestData, let date = Format.date(latest) {
                ...
                let pullStale = market.pullIsBehind(now: now)   // ONE rule, shared with System Health
                if pullStale || daysBehind >= 4 {
                    ...
                    // The TITLE must name the signal that actually fired. It said
                    // "data N days stale" for both, so a skipped pull was reported as
                    // an age the engine itself called fresh — six red errors next to a
                    // System Health page that said every market was Clear.
                    let hours = market.pullHoursAgo(now: now)
                    let why = pullStale
                    ...
                    let tables = (market.staleTables?.isEmpty == false)
                        ...
                        : ""
                    let title = pullStale
                        ? "\(market.market): no pull for \(hours.map { "\($0)h" } ?? "over a day")"
                        : "\(market.market): data \(daysBehind) days stale"
            ...
            // The day history has its OWN nightly step (daily_metrics.py). It
            // stopped for five nights while the perf pull read fresh, so the
            // dashboard's day grid greyed out with nothing to explain it. The
            // engine decides staleness; the app only reports what it says.
            if let history = market.dailyTotals, history.stale {
                let behind = history.behindDays.map { "\($0) days" } ?? "days"
                out.append(.live(.error, .stale, key: "dayhistory:\(market.market)",
                                 title: "\(market.market): day history is \(behind) behind",
                                 detail: "Banked days stop at \(history.last ?? "unknown"). "
                                       + "The dashboard's day grid, trend, month and year-to-date "
                                       + "rows all read this, so they are short. It is filled by "
                                       + "daily_metrics.py, a different step from the pull.",
                                 market: market.market,
                                 fix: .operatorCommand("ADS_MARKET=\(market.market) python3 engine/daily_metrics.py"),
                                 at: now))
            }
        ...
        // A run that covered fewer markets than are configured is NOT a failed
        // step: the loop finishes everything it started, so ok stays true and
        // failures stays empty. That is exactly how five nights (2026-08-16 →
        // 08-20) advertised US alone while the status file said "all steps OK".
        // Nothing else in the app can see it, so it has to be an issue here.
        if let run = health?.lastRun, run.ok {
            let configured = (health?.markets ?? []).filter(\.configured).map(\.market)
            let skipped = run.skippedMarkets(configured: configured)
            if !skipped.isEmpty {
                out.append(.live(.warning, .health, key: "lastRunMarkets:\(run.finished ?? "unknown")",
                                 title: "Nightly run skipped \(skipped.count) market\(skipped.count == 1 ? "" : "s")",
                                 detail: "It ran \(run.markets?.joined(separator: " ") ?? "—") and skipped "
                                       + "\(skipped.joined(separator: " ")). Every step it ran passed, so the run "
                                       + "reported OK — the market list is discovered once at start-up.",
                                 fix: .operatorCommand("grep 'markets:' outputs/scheduled_runs.log | tail -5"),
                                 at: now))
            }
        }
    ...
    static func alertRoute(_ alert: EngineAlert) -> Route {
        ...
        case "portfolio_cap":
            ...
            return .screen(.dashboard)
        case "rules_lost":
            // rule_defs stopped loading, so the nightly evaluates nothing.
            // Rules is where the (now empty) list is, and where the operator
            // sees that everything they authored is gone.
            return .screen(.rules)
        case "stream_undercount":
            // Stream delivered a whole day and it did not match the report.
            // System Health is where the drain, the queues and the backlog all
            // are, so it is where the cause will be.
            return .screen(.health)
        case "stream_check_failed":
            // The undercount check itself raised, so nothing is watching for
            // Stream quietly dropping data. Same destination as the failure it
            // was meant to catch.
            return .screen(.health)
        case "guard_check_failed":
            // One of the detectors that watch for operator DATA going missing
            // raised instead of answering. Nothing is watching that thing now,
            // and an empty alerts feed is exactly what a healthy account looks
            // like — which is the whole reason this kind exists.
            //
            // System Health is the destination: it is the one screen about the
            // machinery rather than about the ads, and the message names which
            // check stopped.
            return .screen(.health)
        case "stream_db_corrupt":
            // The Stream database failed its own integrity check. Hours already
            // banked may be unreadable and Stream never resends, so the cost
            // grows with every drain that writes over more of it. System Health
            // is where the drain, the queues and the subscription state are.
            return .screen(.health)
        case "aws_plan_expiry":
            // The AWS account holding the Marketing Stream queues auto-closes at
            // the end of its free plan. That failure is silent by design of the
            // parts involved: the queues go, Stream stops arriving, and Amazon
            // keeps reporting the subscription ACTIVE. Every screen still works
            // and the day just reads quieter. System Health is where the queues,
            // the drain and the subscription state are.
            return .screen(.health)
        case "seasonal_tags_lost":
            // The tag map went empty after designs had been tagged, so the
            // scheduler is doing nothing. Seasonal is where the count is shown
            // and where the tags are put back.
            return .screen(.seasonal)
        default:
            return .screen(.dashboard)
        }
    }

    /// "93 timed out, 41 deleted" — the biggest reasons first, at most three,
    /// with underscores turned back into words.
    static func removedSummary(_ statuses: [String: Int]?) -> String {
        guard let statuses, !statuses.isEmpty else { return "removed" }
        let parts = statuses.sorted { ($0.value, $1.key) > ($1.value, $0.key) }
            .prefix(3)
