...
struct StatCard: View {
    ...
    var symbol: String?
    var subtitle: String?
    /// Colour for `subtitle`, defaulting to the usual muted footnote.
    ///
    /// Set it when the subtitle is not a footnote but a QUALIFICATION — "this
    /// number covers a shorter window than the cards beside it". That sentence
    /// has to be seen to do its job, and muted grey is what a reader skips.
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            if let delta {
                ...
                Text(subtitle)
                    ...
                    .foregroundStyle(subtitleTint ?? Theme.Colors.muted)
            }
        }
        .padding(.vertical, 16)
