...
struct Market: Codable, Identifiable, Hashable {
    ...
    static let fallback: [Market] = [
        Market(code: "US", currency: "USD", region: "NA", isDefault: true, tamas: true, hasData: true),
        Market(code: "UK", currency: "GBP", region: "EU", isDefault: false, tamas: false, hasData: true),
        Market(code: "DE", currency: "EUR", region: "EU", isDefault: false, tamas: false, hasData: true),
        Market(code: "FR", currency: "EUR", region: "EU", isDefault: false, tamas: false, hasData: true),
        Market(code: "ES", currency: "EUR", region: "EU", isDefault: false, tamas: false, hasData: true),
        Market(code: "IT", currency: "EUR", region: "EU", isDefault: false, tamas: false, hasData: true),
    ]
}
...
struct HaloDesign: Codable, Identifiable, Hashable {
    ...
    /// descriptive ad-group name. A few TAMAS-only designs never got a descriptive
    /// ad group, so they have no title — those fall back to the campaign keyword,
    /// still tagged with the ASIN so the design stays identifiable. The "TAMAS - "
    /// prefix is legacy (that method was removed 2026-08-06, but its paused
    /// campaigns still carry the name in banked history).
    var label: String {
        if let title, !title.isEmpty { return "\(title) — \(asin)" }
        let keyword = (name ?? asin)
            .replacingOccurrences(of: "TAMAS - ", with: "")
            .replacingOccurrences(of: " - \(asin)", with: "")
        return keyword == asin ? asin : "\(keyword) — \(asin)"
    }
}
