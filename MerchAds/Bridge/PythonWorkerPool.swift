...
actor PythonWorkerPool {
    ...
    /// A change to ANY of these retires every worker. dataRoot is part of it
    /// because a worker's data folder is fixed at spawn: it reaches the engine
    /// as an environment variable, and changing the folder in Settings has to
    /// replace the processes, not just the next request.
    ...
    func call(python: String, engineRoot: URL, dataRoot: URL,
              args: [String], market: String?) async throws -> Data {
        let current = Config(python: python, engineRoot: engineRoot.path, dataRoot: dataRoot.path)
        if config != current {
        ...
        let key = market ?? ""
        let worker = try await pick(key: key, python: python, engineRoot: engineRoot,
                                    dataRoot: dataRoot, market: market)
        do {
    ...
    private func pick(key: String, python: String, engineRoot: URL, dataRoot: URL,
        ...
        if (workers[key] ?? []).count < Self.maxWorkersPerMarket {
            let fresh = try Worker(python: python, engineRoot: engineRoot,
                                   dataRoot: dataRoot, market: market)
...
private actor Worker {
    ...
    init(python: String, engineRoot: URL, dataRoot: URL, market: String?) throws {
        let process = Process()
        process.executableURL = URL(fileURLWithPath: python)
        // Same resolver the one-shot bridge uses, so both layouts agree.
        let appctl = PythonBridge.appctlURL(under: engineRoot)
            ?? engineRoot.appendingPathComponent("engine/appctl.py")
        process.arguments = [appctl.path, "serve"]
        // The DATA folder, not the engine's. outputs/, KILL and NO_DISCORD are
        // all resolved relative to the working directory.
        process.currentDirectoryURL = dataRoot
        // Shared with the one-shot path, so the two can no longer disagree
        // about where the databases are.
        process.environment = PythonBridge.engineEnvironment(dataRoot: dataRoot, market: market)
        let inPipe = Pipe()
        let outPipe = Pipe()
