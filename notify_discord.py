...
# --- separate entities: TAMAS + scavenger (name-prefixed buckets) ---
def bucket(prefix):
    r = cur.execute(
...
tn, tc, ts, to, tacos = bucket("TAMAS - ")
sn, sc, ss, so, sacos = bucket("SCAVENGER - ")
tamas_acts = f"{applied.get('tamas_bid',0)} bid moves · {applied.get('tamas_pause',0)} pauses"
...
if MKT == markets.DEFAULT:   # TAMAS is US-only
    fields.append({"name": "🎯 TAMAS",
                   "value": f"{tn} camps · {SYM}{tc:,.0f} · {to} orders · ACOS {tacos:.0f}%\n{tamas_acts}", "inline": True})
