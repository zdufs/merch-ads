...
scavenger, standard, harvested. (It began life as a TAMAS-only tool, keyed on
campaigns that held exactly one ASIN. That shape does not generalise: a lottery
campaign holds up to 1,000 ASINs. So the unit here is the DESIGN, and its ad facts
are summed across every ad group that advertises it.)
...
def _types_by_asin(conn):
    """{asin: sorted set of campaign-name-derived types} using the same classifier
    the rest of the engine uses for campaign kind."""
    import scavenger
    import tamas
    out = {}
    for asin, cname in conn.execute("""
        ...
        if not asin:
            continue
        low = (cname or "").lower()
        if tamas.is_tamas(cname):
            kind = "tamas"
        elif scavenger.is_scavenger(cname):
            kind = "scavenger"
        elif "lotto" in low or "lottery" in low:
            kind = "lottery"
        elif "harvest" in low:
            kind = "harvested"
        else:
            kind = "standard"
        out.setdefault(asin, set()).add(kind)
