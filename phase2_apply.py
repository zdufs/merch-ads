...
def candidates(conn):
    ...
    # campaigns that run on their OWN optimizers — keep the standard rules off them
    skip_ids = {r[0] for r in cur.execute("SELECT campaign_id,name FROM campaigns")
                if tamas.is_tamas(r[1]) or scavenger.is_scavenger(r[1])}
    ...
    already = {(str(agid), db.detail_prefix(term)) for agid, term in cur.execute(
        "SELECT entity_id, detail FROM writes_log WHERE action='add_negative'")}
...
def apply_pauses(client, conn, pauses):
    ...
    for r in pauses:
        ...
        db.log_write(conn, "pause_ad_group", "adGroup", r[0],
                     detail, "ENABLED",
                     "failed" if str(r[0]) in rejected else "submitted")
    db.set_local_ad_group_state(conn, [i for i in ids if str(i) not in rejected], "PAUSED")
    ok = sum(1 for b in res if b["http"] in (200, 207))
...
def rollback_pauses(client, conn):
    rows = conn.execute(
        "SELECT DISTINCT entity_id FROM writes_log WHERE action='pause_ad_group'").fetchall()
    ...
    for pid in ids:
        db.log_write(conn, "rollback_enable", "adGroup", pid, "rollback", "PAUSED",
                     "failed" if str(pid) in rejected else "submitted")
    db.set_local_ad_group_state(conn, [i for i in ids if str(i) not in rejected], "ENABLED")
    ok = sum(1 for b in res if b["http"] in (200, 207))
    print(f"  done ({ok}/{len(res)} batches accepted"
          + (f", {len(rejected)} item(s) REJECTED" if rejected else "") + ").")
