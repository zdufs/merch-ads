...
Adjusts the auto targeting-group bids toward each product's target ACOS:
  ...
  - thin data / 0 sales      -> leave alone (negatives & pauses handle those)
...
def build_proposals(conn, client, min_clicks):
    ...
    active = [r[0] for r in cur.execute(
        ...
        if r[0] not in skip_ids]   # TAMAS + scavenger run on their own optimizers
    if not active:
        return end, []
...
def apply(client, conn, proposals, end):
    ...
    for p in proposals:
        db.log_write(conn, "bid_change", "target", p["targetId"],
                     f"snap={end} {p['old']}->{p['new']} ({p['reason']})"
                     + (p.get("econ_sfx") or ""), str(p["old"]),
                     "failed" if str(p["targetId"]) in rejected else "submitted")
    db.set_local_target_bids(
        conn, [(p["targetId"], p["new"]) for p in proposals
               if str(p["targetId"]) not in rejected])
