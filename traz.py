#!/usr/bin/env python3
"""
TRAZ + EPC — the TAMAS profitability metric. Read-only.
  TRAZ = total royalty (ad + ORGANIC, from the Merch export) − ad spend (ads data)
  EPC  = TRAZ ÷ ad clicks
Per ASIN, over the last-30-day window. This is the metric the TAMAS method judges
...
load_asin_royalty() for the TAMAS optimizer.
...
# split by market. Used by the TAMAS halo estimator; the optimizer still uses the
...
def load_sales_rows(path=None):
    ...
    accumulated per day (sales_import), so halo, TAMAS candidates and TRAZ all
