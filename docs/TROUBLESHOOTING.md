...
1. **The refresh token is wrong or expired.** Rerun `python3 get_token.py`.
...
wrong one. Re-run `python3 list_profiles.py` and copy the ids again.
...
```bash
ADS_MARKET=ES python3 phase0_pull.py
ADS_MARKET=ES python3 daily_metrics.py
```
...
```bash
python3 appctl.py history-import ~/Downloads/history.csv
```
...
1. **Kill switch on?** `python3 appctl.py kill`
2. **Approval mode on?** `python3 appctl.py approval-mode` — changes are waiting in the
   Approval Queue.
3. **Data stale?** `python3 appctl.py health` — a stale table freezes writes on purpose.
4. **Economics gate closed?** `python3 appctl.py econ-gate` — the reply lists reasons.
   Usually a price export older than 21 days.
5. **Nothing actually qualified.** Run the phase in preview and read the output:
   `ADS_MARKET=US python3 phase3_bids.py`
...
```bash
python3 appctl.py maxbid
python3 appctl.py maxbid --set --target 0.75
```
...
If `econ` says unavailable, run `python3 map_products.py` and check
`python3 appctl.py econ-gate`.
...
3. `python3 appctl.py health` output, **with your profile ids and any ASINs removed**.
