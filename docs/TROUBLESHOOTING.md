...
Nothing is configured yet. The engine says so in a sentence rather than raising, because
a fresh install genuinely has no credentials:

```bash
cp .env.example .env      # then fill it in
```

Until it exists, every command that talks to Amazon refuses with that message and every
local read still works. That is the intended state, not a fault.
...
### `stream_check_failed` alert

The Stream undercount check itself raised an exception, so **nothing is currently
watching for Stream quietly dropping data**. Every other Stream check only proves that
what arrived was read faithfully.

```bash
ADS_MARKET=US python3 engine/appctl.py stream-verify
```

That prints the same error with a stack behind it. A market with no Stream data does not
cause this — it reports `comparable: false` with a reason instead — so this alert means a
real fault, usually a schema change.

The alert is keyed on the exception type, so a persistent fault appears once rather than
on every poll.

### `aws_plan_expiry` alert

The AWS account holding the Stream queues is nearing its closing date. It is on the free
plan, which auto-closes six months after it was opened.

The bill for two SQS queues is about nothing, so this is paperwork — and it matters
because of **how** it fails. If the account lapses the queues go, Stream stops arriving,
and Amazon carries on reporting the subscription `ACTIVE`. Every screen keeps working.
The day just reads quieter, which is indistinguishable from a slow sales week.

The alert starts **60 days out** and keeps speaking after the date. Upgrade the AWS
account, then update `AWS_PLAN_EXPIRY` in `engine/stream_config.py`.
Setting it to `None` switches the warning off deliberately.

### A rules run applied nothing and says `change_volume`

A rule proposed more changes than the per-market cap (500 by default), so **nothing was
applied**. That is the guard working: a normal night is tens of changes.

```bash
ADS_MARKET=US python3 engine/appctl.py rules-preview --rule "The rule"
```

Read what it wanted to do. Then either fix the rule, set it to REVIEW mode and approve
from the queue, or raise the cap with `appctl.py change-cap --set N`.
...
## Marketing Stream problems

### The subscription says `ACTIVE` but nothing ever arrives

Almost always the **queue access policy naming the wrong publisher**. Each dataset is
published from a different Amazon AWS account, so a policy written for `sp-traffic` and
reused on the `sp-conversion` queue drops every message — silently, with no error
anywhere and the subscription still reading `ACTIVE`.

Regenerate the correct policy and paste it over:

```bash
ADS_MARKET=US python3 engine/appctl.py stream-setup --queue-url <the queue URL>
```

### The subscription is stuck at `PENDING`

The SNS handshake has not been answered. Creating a subscription parks a
`SubscriptionConfirmation` message in your queue, and Amazon sends nothing else until
something confirms its token. Run a drain — it answers automatically:

```bash
python3 engine/stream_drain.py --seconds 30
```

It prints `SNS handshake confirmed` when it does. If the token has expired (they last a
few days), archive the subscription and create it again.

### `stream-subscribe` fails with a 400

Two shapes are common and both name the problem:

- `Value null at 'clientRequestToken'` — an old build. Update; the engine sends one.
- `Invalid prefix. Must start with 'amzn{version}.'` — the `--subscription` id is wrong.
  Take it from `stream-status`, not from a queue or topic name.

### The queue is filling up

Nothing is draining it. Check the hourly job:

```bash
launchctl print gui/$(id -u)/io.github.zdufs.merchads.stream | grep -E 'state|last exit'
tail -20 outputs/stream_drain.log
```

SQS keeps messages **14 days** if the queues were created with retention raised, and only
**4 days** at the AWS default. Past that, messages are deleted and Stream will not resend
them. Raise retention on the queue if it is still at 4 days.

### `stream-status` says `aws_configured: false`

`AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` are missing from `.env`. Without them there
is no way to read the queue. See [marketing-stream.md](marketing-stream.md).

### `SignatureDoesNotMatch` from AWS

The clock. AWS rejects a signature more than five minutes out. Check the Mac's time is
set automatically. If the clock is fine, run `python3 -m unittest tests.stream_tests` —
it pins the signer against AWS's own published test vector.
...
**Rebuild. Every change needs it, Python included.**
...
This page used to say engine changes took effect on relaunch, because the app shelled out
to your checkout. That stopped being true on 2026-08-21, when the app became standalone:
it carries its own copy of the modules at `Contents/Resources/engine` and runs **those**.
A Python fix sits in the repo, passing its tests, while the app goes on running the old
code — and nothing looks wrong.

If you work in this repo with Claude Code, the Stop hook now hashes everything the bundle
ships (Swift, `engine/**.py`, the two shell scripts) and refuses to end a turn on an
unpackaged change.
...
**If the test run never finishes**, use `python3 tests/run_all.py` instead. It runs the
same tests with a watchdog: after five minutes it prints every thread's stack and exits,
so the hang names a file and a line instead of leaving only an exit code.

Six tests shell out to `appctl` against the real market database. With the app running,
one of those children can block on a SQLite lock. Each now has a 60-second timeout, so
the symptom is a named test error rather than a suite that never returns. Closing the
app before a test run avoids it entirely.

Expect the run to end in `OK` — the count grows every release, so read the last line
rather than matching a number. Tests need no credentials and no network; they use
