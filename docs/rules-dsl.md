...
**A ratio with a zero denominator is `NONE`**, and `NONE` matches no numeric
comparison — so the rule skips that row rather than acting on a number nobody
measured:

| field | zero denominator | value |
|---|---|---|
| `acos` | no sales | `NONE` |
| `cpc` | no clicks | `NONE` |
| `roas` | no spend | `NONE` |
| `cvr` | no clicks | `NONE` |
| `ctr` | no impressions | `NONE` |

A measured zero is different and still counts. An ad group with 40 clicks and
no orders has a real `cvr` of `0`, and `IF adGroup.cvr < 8%` matches it. One
with no clicks at all has no conversion rate to compare, and is skipped.

`cvr` and `ctr` used to answer `0` here, which is not "unknown" — it is the
worst possible score, on exactly the two metrics you write with `<` because low
is bad. `IF adGroup.cvr < 8%` was therefore TRUE for every ad group that had
never been clicked: 37,330 of them on a live account, against a rule whose
action is `pause()`. Fixed 2026-08-23.

**`NONE` does not match a NEGATED test either.** This is the half that is easy
to get wrong. `IF target.cvr != 0` reads like "has some conversion rate", and on
a row with no clicks the honest answer is "nobody knows" — not "yes". So a
comparison against `NONE` answers UNKNOWN, and UNKNOWN travels through `AND`,
`OR` and `NOT` the way it does in SQL before the rule decides:

| condition | row with no clicks | why |
|---|---|---|
| `cvr < 8%` | no match | unknown is not below anything |
| `cvr != 0` | no match | unknown is not different from anything either |
| `cvr NOT IN [0]` | no match | same |
| `NOT (cvr < 8%)` | no match | negating "unknown" is still unknown |
| `clicks >= 15 AND cvr < 8%` | no match | the floor already decided it |

Until 2026-08-23 the first row was right and the rest were matches, because the
negations were built as `not (equals)` and "unknown equals zero" is false. A
rule reading `IF adGroup.cvr != 0: adGroup.pause()` would have paused every ad
group nobody had clicked. No shipped rule used any of these, and all 56 rule
previews across all seven markets were identical before and after the fix.

**Asking about `NONE` on purpose still works.** `IF target.bid != NONE` means
"is a bid set" and answers properly — writing the literal yourself is a real
question about missing data, and it keeps its real answer. That holds through a
`LET` binding too, and inside a list: `IF x IN [NONE]` asks the same question.

Only `=`, `!=`, `IN` and `NOT IN` can answer it. `bid < NONE` is not a question
about missing data, it is nonsense, so it reads UNKNOWN — and so does
`NOT (bid < NONE)`, which would otherwise invert a refusal into a match.

**Functions and text are strict.** One UNKNOWN argument makes the answer
UNKNOWN, so none of these matches a row with no data:

| written | with `x` unknown |
|---|---|
| `IF(x != 0, FALSE, TRUE)` | UNKNOWN — neither branch is picked |
| `LOWER(x)` `UPPER(x)` `CONCAT(x, "")` | UNKNOWN |
| `x + ""` | UNKNOWN |
| `MAX(x, $0.10)` `ROUND(x, 2)` | UNKNOWN |

Until 2026-08-23 those produced the four characters `None` instead, because a
missing value went through Python's `str()` on the way. `IF(...)` picked its
else-branch and answered TRUE. So a rule reading

```
FOR EACH adGroup:
  IF IF(adGroup.cvr != 0, FALSE, TRUE):
    adGroup.pause()
```

proposed a live pause on a design nobody had measured, and the trace behind it
was empty.

**A clicks floor is still worth writing.** `NONE` protects you from the
never-clicked rows; it does not protect you from acting on one click and no
order. Every shipped rule sets a floor — `pause-kill-candidates` needs
`clicks >= 15` before it looks at `cvr` at all.
...
- **On demand**: "Run & apply now" (app) or `ADS_MARKET=<m> python3 engine/appctl.py rules-run
