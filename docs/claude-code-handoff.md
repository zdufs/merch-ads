...
- The engine convention is per-market via the `ADS_MARKET` env var (default US). The app
  must set it when calling `appctl.py`, e.g. `ADS_MARKET=DE python3 appctl.py metrics`.
...
- `markets` → `{markets:[{code,currency,region,is_default,tamas,has_data}], current}`
...
- `campaigns [--type standard|lottery|scavenger|tamas|harvested] [--state ENABLED|PAUSED]`
...
- `halo` → TAMAS organic-halo estimate (US-only; `supported:false` elsewhere):
  `{market,report_start,report_end,supported,note, designs:[{asin,name,ad_start,ad_spend,
  ad_clicks,total_royalty,net_units,pre_days,post_days,pre_royalty,post_royalty,base_rate,
  post_rate,halo_est,net_halo,traz_window,flags}]}`. Windows the dated Merch
  `SALES_REPORT-*.csv` (total royalty, all channels — richer than the export's static
  `royaltyLast30`) to each TAMAS design's ad-serving period and estimates the INCREMENTAL
  organic lift over that design's own pre-ad baseline: `base_rate`/`post_rate` are $/day
  royalty before/after `ad_start` (first impressions), `halo_est = (post_rate−base_rate)×
  post_days`, `traz_window` = post-window royalty − ad_spend. **`halo_est` is an UPPER
  BOUND — correlational, not causal**; `flags` marks `never-served (control)`,
  `no-ad-traffic (0 clicks)`, `peak-before-ad (baseline-confound)`. Backed by
  `tamas_halo.py` + `traz.load_asin_royalty_windowed`; also writes `outputs/tamas_halo.csv`.
- `tamas-candidates [--all --min-units N --min-sale-days N --min-span N --max-prime-spend X]`
  → proven organic sellers eligible for TAMAS (US-only; `supported:false` elsewhere):
  `{market,report_start,report_end,supported,note,thresholds,prime_count,flagship_count,
  prime:[…],flagship:[…]}` where each design = `{asin,title,product_type,net_units,royalty,
  sale_days,span_days,organic_per_day,ad_spend,ad_orders,in_ads,flags}`. Mines the dated
  Merch `SALES_REPORT` for consistent multi-week organic demand (defaults: tee-only,
  net≥8u, ≥6 sale-days, ≥21d span, royalty>0), excludes designs already on TAMAS, and tiers
  by current ad spend — **prime** (< `max_prime_spend`, under-advertised = most upside) vs
  **flagship** (already advertised, strong organic). `flags` marks `sensitive` (weed/suicide
  themes Amazon may restrict for keyword ads). Keyword + price choice stay MANUAL per the
  method; do NOT drop proven sellers to the $13.99 test price. Backed by `tamas_candidates.py`;
  also writes `outputs/tamas_candidates.csv`.
- `sales-report [--import PATH]` → the dated Merch SALES_REPORT, which is the ONLY
  ...
  other one. Feeds `halo`, `tamas-candidates` and `traz.py`. **App: the TAMAS
  screen's sales-report bar** (picker or drag-drop). `traz.sales_report_path()`
...
## Verify
- Backend: `ADS_MARKET=US python3 appctl.py metrics` (and each cmd) returns valid JSON.
