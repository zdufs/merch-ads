...
- Confirm KENP actually lands. A KDP pull is a live API read, so stage it for
  the operator to run: `ADS_MARKET=USKDP python3 appctl.py run --phase pull`. Then check
...
- `ADS_MARKET=USKDP python3 appctl.py rules-preview` (rule text on stdin) should
