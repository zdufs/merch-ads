...
- **Change cap.** One automatic rules run may apply at most **500 changes per market**.
  Past that it applies **nothing at all** and says why.

  This is the only gate that counts. Every other one asks whether a single change is
  safe, so a condition one character too loose — `>= 1` where `>= 15` was meant —
  matches tens of thousands of rows and all of them wave it through.

  500 was measured, not guessed. Counting only the actions a rule can emit, the busiest
  day on record in any market is 255; a normal night across a whole account is 4 to 49
  writes.

  ```bash
  python3 engine/appctl.py change-cap                 # show the current cap
  python3 engine/appctl.py change-cap --set 800       # raise it
  python3 engine/appctl.py change-cap --set 0         # no cap (deliberate)
  python3 engine/appctl.py change-cap --clear         # back to 500
  ```

  It refuses the whole run rather than applying the first 500, because a partial apply
  leaves the account in a state no rule described and nobody chose. When it fires,
  read the proposals with `rules-preview`, then either fix the rule, switch it to
  REVIEW mode and approve from the queue, or raise the cap.

  **Approving by hand is exempt.** Everything you select in the Approval Queue applies
  however many there are — you already looked at them.
