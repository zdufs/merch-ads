...
```bash
touch KILL
python3 appctl.py kill      # -> {"ok": true, "data": {"kill_active": true}}
rm KILL
```
...
```bash
ADS_MARKET=US python3 phase2_apply.py     # negatives + pauses
ADS_MARKET=US python3 phase3_bids.py      # bid changes
ADS_MARKET=US python3 appctl.py rules-preview --rule "My rule"
```
...
```bash
python3 appctl.py econ-gate      # -> {"ok": false, "reasons": [...]} when closed
```
...
- **Bid ceiling.** Set a per-market maximum bid and daily budget. Every write is clamped
  ...
  ```bash
  python3 appctl.py maxbid --set --target 0.50 --keyword 0.50 --budget 20
  python3 appctl.py maxbid                 # show current ceilings
  python3 appctl.py maxbid --clear
  ```
...
```bash
python3 appctl.py approval-mode --on
```
...
```bash
python3 appctl.py approval-mode --off     # when you trust it
python3 appctl.py approval-mode           # check current state
```
...
```bash
python3 appctl.py audit --limit 50        # newest first
python3 appctl.py undo --row 12345        # reverse one write
```
...
python3 appctl.py audit --limit 50  # 2. see what it did
python3 appctl.py undo --row <id>   # 3. reverse what you want back
```
