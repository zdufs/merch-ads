...
| Install it **on Windows** | **[WINDOWS.md](WINDOWS.md)** |
| Build my own screen, because I have no Mac | **[BUILD-A-UI.md](BUILD-A-UI.md)** |
...
| Get hourly data instead of waiting for reports | [marketing-stream.md](marketing-stream.md) |
...
- **[WINDOWS.md](WINDOWS.md)** — the same, on Windows, through WSL. What runs there,
  what does not, and how to schedule the nightly with Task Scheduler.
...
- **[marketing-stream.md](marketing-stream.md)** — Amazon Marketing Stream: hourly
  figures pushed to a queue you own, instead of polling for a report to be built. Needs
  an AWS account. Does not replace the nightly pull.
...
- **[BUILD-A-UI.md](BUILD-A-UI.md)** — the engine answers 108 commands with one JSON
  object each, and the shipped Mac app uses nothing else. This is the contract, the four
  rules a front end must obey, and a prompt to hand an AI coding agent.
