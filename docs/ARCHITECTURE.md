...
- **TAMAS** (`tamas*.py`) — manufacture a proven organic seller into a bestseller with a
  focused keyword campaign. **US only**: it needs the dated Merch `SALES_REPORT`, which
  has no marketplace column. It is **opt-in by existence**: the nightly optimizer looks
  only at ENABLED campaigns with the `TAMAS - ` name prefix, so if you never create one it
  costs nothing and never calls Amazon.
- **TRAZ / halo** (`traz.py`, `tamas_halo.py`) — estimate the *organic* lift a design got
  while it was advertised, by comparing royalty per day before and after ads started.
  **Correlational, not causal.** `halo_est` is an upper bound and the code says so.
