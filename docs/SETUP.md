...
```bash
ADS_MARKET=US python3 phase0_pull.py
```
...
```bash
ADS_MARKET=US python3 appctl.py campaigns          # your campaigns
ADS_MARKET=US python3 appctl.py killlist           # designs losing money
python3 appctl.py health                           # all markets at once
```
...
```bash
touch KILL
python3 appctl.py kill          # should report kill_active: true
rm KILL
```
...
```bash
ADS_MARKET=US python3 phase2_apply.py     # shows proposed negatives and pauses
ADS_MARKET=US python3 phase3_bids.py      # shows proposed bid changes
```
...
```bash
for M in UK DE FR ES IT; do ADS_MARKET=$M python3 phase0_pull.py; done
```
...
TAMAS, the organic-halo estimate and true profit need your dated Merch `SALES_REPORT`
...
```bash
python3 appctl.py sales-report --import ~/Downloads/SALES_REPORT-....csv
```
...
```bash
python3 appctl.py kdp-book --asin B0XXXXXXX --list-price 12.99 --royalty 4.55
```
