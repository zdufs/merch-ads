...
for M in US UK DE FR ES IT; do ADS_MARKET=$M python3 engine/preempt_negatives.py --apply --auto; done
# reactive negatives on markets that already have data (US, UK):
for M in US UK; do ADS_MARKET=$M python3 engine/phase2_apply.py --apply --auto; done
```
