...
```
ADS_MARKET=UK python3 engine/scavenger_build.py        # preview UK scavenger
ADS_MARKET=UK python3 engine/phase0_pull.py            # pull UK data
```

With no env var everything is **US, exactly as before** (US profile, NA endpoint,
`ads_data.sqlite`, hardcoded US economics).

## What each market gets
Same strategies as US: scavenger discovery, negative
...
```
ADS_MARKET=DE python3 engine/lottery_build.py            # preview
ADS_MARKET=DE python3 engine/lottery_build.py --apply
```
...
| Organic halo | guarded to US only (needs the Merch sales report) |
...
```
python3 engine/list_profiles.py        # prints countryCode + profileId for your account
```
...
```
ADS_MARKET=UK python3 engine/phase0_pull.py        # pull structure + 30-day reports
ADS_MARKET=UK python3 engine/map_products.py       # map ad groups -> product type
ADS_MARKET=UK python3 engine/derive_econ.py        # derive UK royalties/break-even
ADS_MARKET=UK python3 engine/scavenger_build.py    # preview, then --apply to create campaigns
```
