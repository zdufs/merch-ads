...
from rules.evaluator import eval_expr, eval_condition
...
def preview(conn, src, cap=50000, today=None):
    ...
    try:
        ...
        return {"ok": False, "market": markets.current(), "evaluated": 0, "matched": 0,
                "changes": [], "errors": [{"line": e.line, "col": e.col, "message": e.message}]}
    ...
    for fe in prog.rules:
        try:
            ...
            return {"ok": False, "market": markets.current(), "evaluated": evaluated,
                    "matched": matched, "changes": changes,
    ...
    return {"ok": True, "market": markets.current(), "evaluated": evaluated,
    ...
    for stmt in body:
        ...
        if kind == "Let":
            ...
            if stmt.verb == "note":
                ...
                args = [eval_expr(a, scope) for a in stmt.args]
...
def _trace_of(cond, scope):
    ...
    for cmp_node in leaves:
        try:
            actual = eval_expr(cmp_node.left, scope)
            threshold = eval_expr(cmp_node.right, scope)
