...
final class ScreenHelpTests: XCTestCase {
    ...
    func testTheScreensThatCanMisleadCarryTheirCaveat() {
        let required: [Screen: String] = [
            ...
            // These two used to demand a "negatives are permanent" warning. That
            // stopped being true when add_negative became undoable (the engine's
            // UNDOABLE map turns it into remove_negative). The screens tell the
            // truth now, so the caveat they must carry is where the write lands
            // and how it comes back: the Audit Trail.
            .approvals: "Audit Trail",
            .strategyBuilder: "Audit Trail",
            .campaigns: "permanent",
