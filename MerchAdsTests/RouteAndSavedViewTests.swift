...
final class RouteAndSavedViewTests: XCTestCase {
    ...
    func testLegacyKeyMigrationPreservesOldAndWritesIdenticalCanonicalValues() throws {
        ...
        // Only the aliases TableID actually declares are migrated. The test used
        // to also assert a "playbook" -> "playbook.pricing" hop, but no table by
        // that name exists any more: the Playbook screen became Health and its
        // pricing table left with it. Asserting a migration to a key nothing
        // reads kept the whole Swift suite red, which hides real regressions.
