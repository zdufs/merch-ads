...
Use `python3 tests/run_all.py` on a Mac with the app running — it is the same
suite with a watchdog that names a hang instead of waiting forever.

Expect the run to end in `OK`. The count grows every release, so read the last
line rather than matching a number. Loud `BULK WRITE FAILED` lines are simulated
failures inside passing tests — they go to stderr, so they interleave with the
progress dots; read the summary line.
...
**To see a change in the installed app, rebuild the bundle — Python changes included:**
...
Since 2026-08-21 the app is standalone: it carries its own copy of the engine at
`Contents/Resources/engine` and runs **that**, not your checkout. An engine-only fix will
pass every test and never reach the running app until you package. A DerivedData build
ships no engine, so it falls back to the Settings folder and the login-shell `python3` —
which is why those fallbacks must keep working.

`package_app.sh` runs the Swift tests first and refuses to build if they fail. The app
target and the test target compile separately, so the app can build and install perfectly
while the test target does not compile at all.
...
- Swift 6, SwiftUI, system fonts and semantic colours only. See `docs/DESIGN.md`.
- **Put a new Swift file in the folder that matches what it does** — `Views/<Section>/`
  for a screen, `Components/` for something reused, `State/` for observable state,
  `Bridge/` for anything touching `appctl.py` or SQLite. The target uses Xcode 16
  synchronized groups, so creating the file in the right folder is the whole job; there
  is no project file to edit. `Info.plist` and `AppIcon.icns` must stay at the target
  root — a build setting and a resource path point at them.
...
This repository is a snapshot. `scripts/make_public_snapshot.sh` builds it from the
private working repository, drops files that must not ship, runs eleven checks, and only
then commits.

**Do not commit into the published folder by hand.** Rebuild and push. A `pre-push` hook
re-runs every check against the tree being pushed, so a hand-edited tree is refused —
that gate exists because two real leaks reached a commit before the checks caught them:
a launch list of real ASINs, and account revenue figures inside a docstring. Neither was
visible in review; both were obvious to a grep of the built tree.

```bash
bash scripts/make_public_snapshot.sh --owner <github-user> --out ~/merch-ads-public
cd ~/merch-ads-public && git push --force origin main   # hook re-verifies here
```

`git push --no-verify` skips the gate. There is no good reason to use it.

The snapshot commits as your GitHub noreply address rather than whatever your global
git config says, because a commit author is public forever once pushed.
