    ...
    static func main() async {
        ...
        for round in 1...3 {
            await withTaskGroup(of: (String, Bool).self) { group in
                ...
                // Was `nudges`. That endpoint still exists in the engine but no
                // screen reads it any more, so NudgesResponse was removed and
                // this harness stopped compiling. `alerts` is a live read of a
                // different shape, which is what the stampede needs.
                group.addTask {
                    ("alerts", (try? await bridge.call(AlertsResponse.self, ["alerts"], market: "US")) != nil)
                }
                group.addTask {
